-- NYANN OS / MIDNIGHT EDIT
-- UI bootstrap switched to Midnight while the supplied feature code is preserved.
-- NOTE: If Midnight's API is not Redz-compatible, its Source must be adapted
-- to expose the same methods used below.
...
-- =========================================================
-- NYANN OS - MIDNIGHT UI BOOTSTRAP
-- Giữ nguyên toàn bộ code chức năng phía dưới.
-- Midnight được tải trước; adapter cố gắng giữ API Redz:
-- MakeWindow / NewMinimizer / MakeTab / AddButton / AddToggle...
-- =========================================================
local MidnightSource = "https://raw.githubusercontent.com/NIcoGabrielRealYtr/Midnight-Hub-Library/refs/heads/main/Source"

local function LoadMidnight()
    local ok, result = pcall(function()
        local src = game:HttpGet(MidnightSource)
        return loadstring(src)()
    end)
    if not ok then
        warn("[nyann os] Midnight Library load failed: " .. tostring(result))
        return nil
    end
    return result
end

local redzlib = LoadMidnight()
if not redzlib then
    return
end

-- Một số bản Midnight trả trực tiếp Window; một số trả Library.
local Window
if type(redzlib.MakeWindow) == "function" then
    Window = redzlib:MakeWindow({
        Title = "nyann os[Beta] No Key",
        SubTitle = "by real_@nyann",
        SaveFolder = "nyannnaiget.json"
    })
elseif type(redzlib.CreateWindow) == "function" then
    Window = redzlib:CreateWindow({
        Title = "nyann os[Beta] No Key",
        SubTitle = "by real_@nyann",
        SaveFolder = "nyannnaiget.json"
    })
elseif type(redzlib.Window) == "function" then
    Window = redzlib:Window({
        Title = "nyann os[Beta] No Key",
        SubTitle = "by real_@nyann",
        SaveFolder = "nyannnaiget.json"
    })
else
    warn("[nyann os] Midnight không expose API tạo Window tương thích.")
    return
end
