-- [REMOVED] pastefy 404 - không load được, bỏ để menu hiện
...
local okLib, libOrErr = pcall(function()
    return loadstring(game:HttpGet("https://raw.githubusercontent.com/nyannos/test/refs/heads/main/redzlib_with_search-1.lua"))()
end)
if okLib and libOrErr then
    redzlib = libOrErr
else
    warn("[nyann os] Lỗi load redzlib: ", libOrErr)
    -- fallback: thử raw không search
    okLib, libOrErr = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/REDzSTORE/RedzLibV5/main/Redzlib.lua"))()
    end)
    if okLib and libOrErr then
        redzlib = libOrErr
    else
        error("[nyann os] Không load được UI library. Check internet / executor HttpGet.")
    end
end
...
-- ==========================================
-- TAB FONT - Chọn font toàn game + menu
-- ==========================================
v16:AddSection({"Font"})
...
local function ApplyGlobalFont(fontEnum, fontName)
    currentFont = fontEnum
    currentFontName = fontName or tostring(fontEnum)

    -- Áp dụng cho PlayerGui (UI trong game)
    pcall(function()
    ...
    -- Áp dụng cho menu hub (CoreGui)
    pcall(function()
        for _, gui in ipairs(game:GetService("CoreGui"):GetChildren()) do
            if gui:IsA("ScreenGui") then
                for _, obj in ipairs(gui:GetDescendants()) do
                    if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                        -- Không đổi FPS counter nếu muốn giữ style riêng
                        if not (obj.Parent and obj.Parent.Name == "RainbowFPS") then
                            obj.Font = fontEnum
                        end
                    end
                end
    ...
    pcall(function()
        redzlib:Notify({
            Title = "Font",
            Message = "Đã đổi font: " .. currentFontName,
            Duration = 2
        })
    end)
end

-- Tự áp font khi có UI mới spawn
pcall(function()
...
pcall(function()
    for _, fontData in ipairs(fontList) do
        v16:AddButton({
            ...
            Callback = function()
                pcall(ApplyGlobalFont, fontData[2], fontData[1])
            end
        })
    end
end)
