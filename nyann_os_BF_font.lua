...
local function ApplyMenuFont(fontName)
    ...
    if not enumFont then return end
    local parents = {}
    pcall(function()
        ...
        if parent then
            for _, gui in ipairs(parent:GetDescendants()) do
                if gui:IsA("TextLabel") or gui:IsA("TextButton") or gui:IsA("TextBox") then
                    pcall(function() gui.Font = enumFont end)
                end
            end
        end
    end
...
v15:AddDropdown({
    Name = "Select Font",
    Options = FontList,
    Default = GetSetting("MenuFont_Save", "Gotham"),
    Callback = function(Value)
        _G.MenuFont = Value
        _G.SaveData["MenuFont_Save"] = Value
        SaveSettings()
        ApplyMenuFont(Value)
        pcall(function()
            redzlib:Notify({Title = "Font", Message = "Đã đổi font: " .. tostring(Value), Duration = 2})
        end)
    end
})
v15:AddButton({
    Name = "Apply Font Now",
    Callback = function()
        ApplyMenuFont(_G.MenuFont or GetSetting("MenuFont_Save", "Gotham"))
    end
})
-- Apply saved font after menu load
task.delay(1.5, function()
    pcall(function()
        ApplyMenuFont(GetSetting("MenuFont_Save", "Gotham"))
    end)
end)
