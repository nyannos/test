...
pcall(function()
    ...
    if rem then commE = rem:WaitForChild("CommE", 5) end
end)
if not commE then warn("[nyann os] CommE timeout - skip AutoKen remote") end
...
-- Wait for game to load (timeout 20s — tránh treo không hiện menu)
do
    local t0 = tick()
    repeat
...
-- KHÔNG hook error/warn (làm menu fail im lặng)
...
do
    ...
    for _, url in ipairs(urls) do
        ...
        if ok and result then
            ...
            warn("[nyann os] UI lib fail:", url, result)
        end
    end
    if not redzlib then
        error("[nyann os] Cannot load UI library — menu không hiện")
    end
end
...
-- Minimizer: hỗ trợ cả AddMinimizeButton (lib cũ) và NewMinimizer (V5)
pcall(function()
    if Window.AddMinimizeButton then
    ...
    elseif Window.NewMinimizer then
        local m = Window:NewMinimizer({ KeyCode = Enum.KeyCode.LeftControl })
        if m and m.CreateMobileMinimizer then
            m:CreateMobileMinimizer({
                Image = "rbxassetid://94678517792779",
...
-- Ép hiện mọi ScreenGui sau khi tạo menu
task.defer(function()
    task.wait(0.5)
    pcall(function()
        local parents = {}
    ...
    print("[nyann os] Menu forced visible")
end)
