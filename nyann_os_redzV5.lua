...
pcall(function()
    ...
    if r then commE = r:WaitForChild("CommE", 5) end
end)
if not commE then warn("[nyann os] CommE timeout") end
...
-- Không hook error/warn (làm Redz V5 fail im lặng)
...
-- ========================================
-- REDZ LIBRARY V5 (tlredz) + SHIM API cũ
-- ========================================
local redzlib
do
    local urls = {
    ...
    for _, url in ipairs(urls) do
        local ok, result = pcall(function()
            ...
            if type(src) ~= "string" or #src < 500 then error("empty lib") end
    ...
    if not redzlib then
        error("[nyann os] Cannot load Redz V5")
    end
end

-- Shim: Notify trên library
if not redzlib.Notify and type(redzlib) == "table" then
    function redzlib:Notify(opts)
        opts = opts or {}
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = tostring(opts.Title or opts.Name or "nyann os"),
                Text = tostring(opts.Message or opts.Content or ""),
...
local function ShimTab(tab)
    if not tab or tab.__NyannShim then return tab end
    tab.__NyannShim = true

    local rawAddSection = tab.AddSection
    if type(rawAddSection) == "function" then
        function tab:AddSection(a)
            if type(a) == "table" then
                a = a[1] or a.Name or a.Title or "Section"
            end
            return rawAddSection(self, a)
        end
    end

    local rawAddParagraph = tab.AddParagraph
    if type(rawAddParagraph) == "function" then
        function tab:AddParagraph(a, b)
            if type(a) == "table" then
                local title = a.Title or a.Name or a[1] or ""
                local desc = a.Desc or a.Description or a.Content or a[2] or ""
                return rawAddParagraph(self, tostring(title), tostring(desc))
            end
            return rawAddParagraph(self, a, b)
        end
    end

    local rawAddToggle = tab.AddToggle
    if type(rawAddToggle) == "function" then
        function tab:AddToggle(cfg)
            if type(cfg) ~= "table" then return rawAddToggle(self, cfg) end
            return rawAddToggle(self, {
                Name = cfg.Name or cfg.Title or cfg[1],
                Default = cfg.Default or cfg[2] or false,
                Callback = cfg.Callback or cfg[3],
                Flag = cfg.Flag,
    ...
    local rawAddButton = tab.AddButton
    if type(rawAddButton) == "function" then
        function tab:AddButton(cfg)
            if type(cfg) ~= "table" then return rawAddButton(self, cfg) end
            return rawAddButton(self, {
                Name = cfg.Name or cfg.Title or cfg[1],
                Callback = cfg.Callback or cfg[2],
                Description = cfg.Description or cfg.Desc,
            })
        end
    end

    local rawAddDropdown = tab.AddDropdown
    if type(rawAddDropdown) == "function" then
        function tab:AddDropdown(cfg)
            if type(cfg) ~= "table" then return rawAddDropdown(self, cfg) end
            return rawAddDropdown(self, {
                Name = cfg.Name or cfg.Title or cfg[1],
                Options = cfg.Options or cfg[2] or {},
                Default = cfg.Default,
                Multi = cfg.Multi or cfg.MultiSelect,
    ...
    local rawAddTextBox = tab.AddTextBox
    if type(rawAddTextBox) == "function" then
        function tab:AddTextBox(cfg)
            if type(cfg) ~= "table" then return rawAddTextBox(self, cfg) end
            return rawAddTextBox(self, {
                Name = cfg.Name or cfg.Title or cfg[1],
    ...
    local rawAddSlider = tab.AddSlider
    if type(rawAddSlider) == "function" then
        function tab:AddSlider(cfg)
            if type(cfg) ~= "table" then return rawAddSlider(self, cfg) end
            return rawAddSlider(self, {
                Name = cfg.Name or cfg.Title or cfg[1],
                Min = cfg.Min or 0,
                Max = cfg.Max or 100,
    ...
    local rawAddDiscord = tab.AddDiscordInvite
    if type(rawAddDiscord) == "function" then
        function tab:AddDiscordInvite(cfg)
            if type(cfg) ~= "table" then return rawAddDiscord(self, cfg) end
            return rawAddDiscord(self, {
                Name = cfg.Name or cfg.Title,
                Title = cfg.Title or cfg.Name,
                Description = cfg.Description or cfg.Desc or "",
                Logo = cfg.Logo or cfg.Icon or cfg.Banner,
                Banner = cfg.Banner or cfg.Logo,
                Invite = cfg.Invite or cfg.Link or "",
                Icon = cfg.Icon or cfg.Logo,
                Members = cfg.Members,
                Online = cfg.Online,
            })
        end
...
do
    local ok, result = pcall(function()
        return redzlib:MakeWindow({
            ...
            ScriptFolder = "nyannos",
        })
    end)
    if not ok or not result then
        error("[nyann os] MakeWindow fail: " .. tostring(result))
    end
    Window = result
    print("[nyann os] Menu Window OK (Redz V5)")
end

-- Shim MakeTab
do
    local rawMakeTab = Window.MakeTab
    function Window:MakeTab(cfg)
        local tab = rawMakeTab(self, cfg)
        return ShimTab(tab)
    end
end

-- Minimizer (V5)
-- Minimizer Redz V5 (PC + Mobile)
local Minimizer
local MobileButton
pcall(function()
...
-- Bind library Notify -> Window Notify
pcall(function()
    function redzlib:Notify(opts)
        opts = opts or {}
        if Window and Window.Notify then
            return Window:Notify({
                Title = opts.Title or opts.Name or "nyann os",
                Content = opts.Message or opts.Content or "",
                Duration = opts.Duration or 4,
                Image = opts.Image or opts.Icon,
            })
        end
    end
end)
