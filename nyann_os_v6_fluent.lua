...
-- ========================================
-- ========================================
-- FLUENT UI + shim (thay Zeox, giữ nguyên Tabs.* API)
-- ========================================
print("[nyann os] loading Fluent UI...")
...
getgenv().NyannFluentWindow = RealWindow
    ...
    local function ToggleWithSpin()
        ...
        pcall(function()
            if RealWindow and RealWindow.Minimize then
                RealWindow:Minimize()
            elseif RealWindow and RealWindow.Toggle then
                RealWindow:Toggle()
            end
        end)
...
local function MakeTabShim(fluentTab)
    ...
    local function safeSection(name)
        ...
        local ok, sec = pcall(function()
            return fluentTab:AddSection(name)
        end)
    ...
    function tab:AddToggle(cfg)
        ...
        local ret
        pcall(function()
            ret = fluentTab:AddToggle(name, {
                Title = name,
                Description = cfg.Description or cfg.Desc or "",
                Default = def
            })

            if ret and ret.OnChanged then
                ret:OnChanged(function(value)
                    pcall(cb, value)
                end)
            else
                pcall(cb, def)
    ...
    function tab:AddDropdown(cfg)
        ...
        local ret
        pcall(function()
            ret = fluentTab:AddDropdown(name, {
                Title = name,
    ...
    function tab:AddParagraph(a, b)
        ...
        local ret
        pcall(function()
            ret = fluentTab:AddParagraph({
                Title = title,
                Content = desc
            })
        end)

        -- Giữ API SetDesc/SetDescription cũ
        local proxy = {
            _title = title,
            _desc = desc,
            _obj = ret
        ...
        function proxy:Set(t)
            ...
            pcall(function()
                if self._obj and self._obj.SetDesc then
                    self._obj:SetDesc(self._desc)
                end
            end)
        end
        ...
        function proxy:SetDescription(d)
            self:SetDesc(d)
        end
    ...
    function tab:AddDiscordInvite(cfg)
        ...
        local title = tostring(cfg.Title or "Discord")
        local desc = tostring(cfg.Description or cfg.Invite or "")
...
function Window:MakeTab(cfg)
    ...
    if not page then
        warn("[nyann os] Fluent AddTab failed:", title, res)
        return MakeTabShim({
...
function Window:NewMinimizer()
    return {
        CreateMobileMinimizer = function() end
    }
end
...
print("[nyann os] Fluent menu ready")
...
Tabs.Settings:AddSection("Menu Theme")
