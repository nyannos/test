...
-- NYANN OS LOADER
-- ==========================================
shared.LoaderTitle = "Load ...., by real_@nyann"
...
LoaderGui.Name = "NYANN_Loader"
LoaderGui.ResetOnSpawn = false
LoaderGui.Parent = game:GetService("CoreGui")

local LoaderMain = Instance.new("Frame")
LoaderMain.Parent = LoaderGui
LoaderMain.BackgroundColor3 = LoaderConfig.Colors.Main
LoaderMain.BorderSizePixel = 0
LoaderMain.Position = UDim2.fromScale(0.5,0.5)
LoaderMain.AnchorPoint = Vector2.new(0.5,0.5)
LoaderMain.Size = UDim2.fromOffset(0,0)

local Corner = Instance.new("UICorner")
Corner.CornerRadius = UDim.new(0,12)
Corner.Parent = LoaderMain

local Avatar = Instance.new("ImageLabel")
Avatar.Parent = LoaderMain
Avatar.BackgroundTransparency = 1
Avatar.Image = "rbxassetid://94678517792779"
...
UserName.Parent = LoaderMain
UserName.BackgroundTransparency = 1
UserName.Text = "Load..."
UserName.Position = UDim2.fromOffset(75,10)
UserName.Size = UDim2.fromOffset(220,50)
UserName.Font = Enum.Font.GothamBold
UserName.TextColor3 = LoaderConfig.Colors.Title
...
Top.Position = UDim2.fromOffset(30,70)
Top.Size = UDim2.fromOffset(301,20)
Top.Font = Enum.Font.Gotham
Top.Text = "Loader"
Top.TextColor3 = LoaderConfig.Colors.Topic
Top.TextSize = 10
...
Title.Position = UDim2.fromOffset(30,90)
Title.Size = UDim2.fromOffset(301,46)
Title.Font = Enum.Font.Gotham
Title.RichText = true
Title.Text = "<b>"..tostring(LoaderConfig.Name).."</b>"
Title.TextColor3 = LoaderConfig.Colors.Title
Title.TextSize = 14
Title.TextXAlignment = Enum.TextXAlignment.Left
Title.TextTransparency = 1

local BG = Instance.new("Frame")
BG.Parent = LoaderMain
BG.AnchorPoint = Vector2.new(0.5,0)
BG.BackgroundColor3 = LoaderConfig.Colors.LoaderBackground
BG.BorderSizePixel = 0
BG.Position = UDim2.new(0.5,0,0,70)
...
Progress.Parent = BG
Progress.BackgroundColor3 = LoaderConfig.Colors.LoaderSplash
Progress.BorderSizePixel = 0
Progress.Size = UDim2.new(0,0,0,24)
...
Step.Parent = LoaderMain
Step.BackgroundTransparency = 1
Step.Position = UDim2.new(0.5,0,1,-25)
Step.Size = UDim2.new(1,-20,0,20)
Step.AnchorPoint = Vector2.new(0.5,0.5)
Step.Font = Enum.Font.Gotham
Step.TextColor3 = LoaderConfig.Colors.Topic
Step.TextSize = 14
Step.TextXAlignment = Enum.TextXAlignment.Center

local TweenService = game:GetService("TweenService")
local function T(obj,time,props)
    local tw=TweenService:Create(obj,TweenInfo.new(time,Enum.EasingStyle.Linear,Enum.EasingDirection.InOut),props)
    tw:Play(); return tw
end

T(LoaderMain,0.25,{Size=UDim2.fromOffset(346,145)})
task.wait(0.25)
T(Top,0.4,{TextTransparency=0})
T(Title,0.4,{TextTransparency=0})
T(BG,0.4,{BackgroundTransparency=0})
T(Progress,0.4,{BackgroundTransparency=0})

for _,frame in ipairs(shared.LoaderKeyFrames) do
    task.wait(tonumber(frame[1]) or 0)
    local pct=math.clamp(tonumber(frame[2]) or 0,0,100)
    Step.Text=tostring(pct).."%"
    T(Progress,0.5,{Size=UDim2.new(pct/100,0,0,24)})
end

task.wait(0.5)
T(Top,0.3,{TextTransparency=1})
T(Title,0.3,{TextTransparency=1})
T(BG,0.3,{BackgroundTransparency=1})
T(Progress,0.3,{BackgroundTransparency=1})
task.wait(0.35)
T(LoaderMain,0.25,{Size=UDim2.fromOffset(0,0)})
task.wait(0.3)
if LoaderGui and LoaderGui.Parent then LoaderGui:Destroy() end

-- Menu gốc chạy tiếp ngay sau loader.
...
v16:AddSection({"Global Font"})

local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local CurrentFont = Enum.Font.Arial

local SupportedTextClasses = {
    TextLabel = true,
    TextButton = true,
    TextBox = true
}
    ...
    pcall(function()
        ...
        if playerGui then
            for _, obj in ipairs(playerGui:GetDescendants()) do
                if SupportedTextClasses[obj.ClassName] then
...
local function WatchGui(gui)
    ...
    gui.DescendantAdded:Connect(function(obj)
        if SupportedTextClasses[obj.ClassName] then
            task.defer(function()
                pcall(function() obj.Font = CurrentFont end)
            end)
        end
    end)
end
...
LocalPlayer.ChildAdded:Connect(function(child)
    if child:IsA("PlayerGui") then
        WatchGui(child)
        task.defer(function() ApplyGlobalFont(CurrentFont) end)
    end
end)
...
for _, data in ipairs(FontList) do
    local fontName, fontEnum = data[1], data[2]
    v16:AddButton({
        Name = "Font: " .. fontName,
        Callback = function()
            ApplyGlobalFont(fontEnum)
        end
    })
end
...
do -- Universal Sea UI: always show
v2:AddToggle({
