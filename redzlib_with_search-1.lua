...
local redzlib = {
		Themes = {
			...
			["Color Stroke"] = Color3.fromRGB(255, 255, 255), -- VIỀN ĐỎ CAO CẤP
			["Color Theme"] = Color3.fromRGB(255, 255, 255), -- Tông đỏ cho các nút
...
-- ... (Code phía dưới giữ nguyên)
	...
	local ContainerList = {} -- declared early for Search filter
	...
	-- ========== SEARCH BOX (phần tìm kiếm) ==========
	local SearchFrame = Create("Frame", TopBar, {
	...
	Make("Corner", SearchFrame, UDim.new(0, 6))
	...
	local SearchIcon = Create("ImageLabel", SearchFrame, {
	...
	-- Filter GLOBAL: tìm ở MỌI tab, tự nhảy sang tab có kết quả
	local function GetChildSearchText(child)
		local labelText = string.lower(child.Name or "")
		for _, desc in pairs(child:GetDescendants()) do
			if desc:IsA("TextLabel") and desc.Text and #desc.Text > 0 then
				labelText = labelText .. " " .. string.lower(desc.Text)
			end
		end
		return labelText
	end
	...
	local function FilterOptions(query)
		...
		-- Không có query → hiện lại tất cả ở mọi tab
		if query == "" then
			for _, cont in pairs(ContainerList or {}) do
				if cont then
		...
		local firstMatchContainer = nil
		local anyMatch = false

		-- Quét TẤT CẢ container (mọi tab)
		for _, cont in pairs(ContainerList or {}) do
			if cont then
				local contHasMatch = false
				for _, child in pairs(cont:GetChildren()) do
					if child:IsA("Frame") or child:IsA("TextButton") then
						local matchStr = GetChildSearchText(child)
						if string.find(matchStr, query, 1, true) then
							child.Visible = true
							contHasMatch = true
							anyMatch = true
							if not firstMatchContainer then
		...
		-- Tự động chuyển sang tab đầu tiên có kết quả
		if firstMatchContainer and redzlib.Tabs then
			for _, tabData in pairs(redzlib.Tabs) do
				if tabData.Cont == firstMatchContainer and tabData.func then
					-- Force switch: unparent current so Enable/Tabs() runs fully
					if firstMatchContainer.Parent then
						firstMatchContainer.Parent = nil
					end
					pcall(function()
						if tabData.func.Enable then
							tabData.func:Enable()
						end
					end)
					break
		...
		elseif firstMatchContainer then
			-- Fallback nếu chưa có Tabs
			for _, cont in pairs(ContainerList or {}) do
				if cont and cont:IsA("ScrollingFrame") then
					cont.Parent = (cont == firstMatchContainer) and Containers or nil
				end
			end
		end
	end
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddToggle(Configs)
			...
			Make("Stroke", ToggleHolder, {Thickness = 1.2}, Color3.fromRGB(255, 255, 255))
			...
			local WaitClick
			local ToggleTitle = TName
			local function UpdateToggleTitle()
				LabelFunc:SetTitle(ToggleTitle .. " [" .. (Default and "ON" or "OFF") .. "]")
			end
			local function SetToggle(Val)
				if WaitClick then return end
				
				WaitClick, Default = true, Val
				Toggle.BackgroundColor3 = Default and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(70, 70, 80)
				UpdateToggleTitle()
		...
		function Tab:AddDropdown(Configs)
			...
			Make("Stroke", SelectedFrame, {Thickness = 1.2}, Color3.fromRGB(255, 255, 255))
