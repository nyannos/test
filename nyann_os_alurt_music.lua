...
pcall(function()
    ...
    if remotes then commE = remotes:WaitForChild("CommE", 5) end
end)
...
-- Wait for game to load (timeout 20s)
do
    local t0 = tick()
    repeat
...
-- ========================================
-- MENU MUSIC (Information) — bật/tắt
-- ========================================
Discord:AddSection({"Music Player"})

local function StopNyannMusic()
    pcall(function()
        local SS = game:GetService("SoundService")
        for _, s in ipairs(SS:GetChildren()) do
            if s:IsA("Sound") and (s.Name == "FluentMusicPlayerSound" or s.Name == "FluentCustomMusic") then
                s:Stop()
                s:Destroy()
            end
        end
    ...
    -- cố gắng ẩn/xóa window music nếu còn
    pcall(function()
        local parents = {}
        ...
        if lp and lp:FindFirstChild("PlayerGui") then table.insert(parents, lp.PlayerGui) end
        for _, parent in ipairs(parents) do
            for _, gui in ipairs(parent:GetChildren()) do
                if gui:IsA("ScreenGui") then
                    local n = string.lower(gui.Name)
                    if n:find("music") or n:find("fluent") or n:find("redz") then
                        -- chỉ xóa gui music nếu title có Menu Music — an toàn hơn: đánh dấu
                        if gui:GetAttribute("NyannMusicMenu") then
                            gui:Destroy()
                        end
                    end
                end
...
Discord:AddToggle({
    ...
    Callback = function(Value)
        if Value then
            if _G.__NyannMusicRunning then
                pcall(function()
                    if redzlib and redzlib.Notify then
                        redzlib:Notify({Title = "Music", Message = "Menu Music đang bật", Duration = 3})
                    end
                end)
                return
            end
            _G.__NyannMusicRunning = true
            task.spawn(function()
                local ok, err = pcall(function()
                    local fn, cerr = loadstring(MUSIC_SOURCE)
                    if not fn then error(cerr or "compile music fail") end
                    fn()
                end)
                if not ok then
                    _G.__NyannMusicRunning = false
                    warn("[nyann os] Music menu error:", err)
                    pcall(function()
                        if redzlib and redzlib.Notify then
                            redzlib:Notify({Title = "Music", Message = "Lỗi mở music: " .. tostring(err), Duration = 5})
                        end
                    end)
                else
                    pcall(function()
                        if redzlib and redzlib.Notify then
                            redzlib:Notify({Title = "Music", Message = "Menu Music đã mở", Duration = 3})
                        end
                    end)
                end
            end)
            ...
            pcall(function()
                if redzlib and redzlib.Notify then
                    redzlib:Notify({Title = "Music", Message = "Đã tắt / dừng nhạc", Duration = 3})
                end
            end)
        end
    end
...
Discord:AddButton({
    ...
    Callback = function()
        ...
        pcall(function()
            if redzlib and redzlib.Notify then
                redzlib:Notify({Title = "Music", Message = "Đã dừng mọi nhạc nyann", Duration = 3})
            end
        end)
    end
})
