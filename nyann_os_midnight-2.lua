...
do
  local urls = {
    "https://raw.githubusercontent.com/nyannos/test/refs/heads/main/Source_midnight_nyann.lua",
  }
...
-- Shim section defaults per page
local function MakeTabShim(page)
  local section = nil
  pcall(function()
    section = page:Section({ Name = "Main", Side = 1 })
  end)
  if not section then
    pcall(function() section = page:Section({ Name = "Main" }) end)
  end
  ...
  function tab:AddButton(cfg)
    ...
    pcall(function()
      ...
      if btn and btn.Add then
        btn:Add(cfg.Name or cfg.Title or "Button", cfg.Callback or function() end)
        ret = btn
      end
    end)
    return ret
  ...
  function tab:AddDropdown(cfg)
    ...
    pcall(function()
      ret = sec:Dropdown({
        ...
        Items = cfg.Options or cfg.Values or cfg.Items or {},
        Default = cfg.Default,
  ...
  function tab:AddParagraph(a, b)
    ...
    if b and tostring(b) ~= "" then text = text .. " | " .. tostring(b) end
    local ret
    pcall(function()
      ret = sec:Label(text)
    end)
    return ret or { Set = function() end, SetDesc = function() end, SetDescription = function() end }
  end
  ...
  function tab:AddTextBox(cfg)
    ...
    pcall(function()
      ret = sec:Textbox({
        ...
        Placeholder = cfg.Placeholder or cfg.PlaceholderText,
        Callback = cfg.Callback or function() end,
        Flag = cfg.Flag,
      })
    end)
  ...
  function tab:AddDiscordInvite(cfg)
    ...
    local sec = ensureSection()
    pcall(function()
      sec:Label(tostring(cfg.Title or cfg.Name or "Discord"))
      if cfg.Invite then sec:Label(tostring(cfg.Invite)) end
      if cfg.Description then sec:Label(tostring(cfg.Description)) end
    end)
  end
...
function Window:MakeTab(cfg)
  ...
  local icon = cfg.Icon or cfg[2]
  local page
  local ok, res = pcall(function()
    return RealWindow:Page({
      Name = tostring(title),
      Icon = icon,
      Columns = 1,
    })
...
function Window:Notify(opts)
  ...
  pcall(function()
    game:GetService("StarterGui"):SetCore("SendNotification", {
      ...
      Text = tostring(opts.Content or opts.Message or ""),
...
-- redzlib alias for Notify calls in script
local redzlib = {
