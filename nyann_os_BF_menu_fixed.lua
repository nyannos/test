-- FPS / extra UI (optional, pastefy may die)
pcall(function()
...
do
    ...
    for _, url in ipairs(urls) do
        local ok, result = pcall(function()
            ...
            if type(src) ~= "string" or #src < 200 then error("empty") end
        ...
        if ok and result then
            redzlib = result
            print("[nyann os] Redz loaded from:", url)
            break
        else
    ...
    if not redzlib then
        error("[nyann os] Cannot load Redz Library - all URLs failed")
    end
end
...
-- Bảo vệ menu: không bị hub/script khác tắt hoặc xóa
task.spawn(function()
    task.wait(1)
                ...
                if parent then
                    for _, gui in ipairs(parent:GetChildren()) do
                        if gui:IsA("ScreenGui") then
                            ...
                            if n:find("redz") or n:find("nyann") or n:find("library") then
                                gui.Enabled = true
