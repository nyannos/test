...
do
  local t0 = tick()
  repeat
    local loading = plr.PlayerGui and plr.PlayerGui:FindFirstChild("Chính")
...
Marines = function() replicated.Remotes.CommF_:InvokeServer("SetTeam","Hải quân") end
Pirates = function() replicated.Remotes.CommF_:InvokeServer("SetTeam","Hải tặc") end
...
local DungeonTables = {"Flame","Ice","Quake","Sáng","Tối","String","Rumble","Magma","Human: Buddha","Sand","Bird: Phoenix","Dough"}
...
Attack.Kill = function(model,Succes)
  ...
  if ToolTip == "Trái ác quỷ" then _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,10,0) * CFrame.Angles(0,math.rad(90),0)) else _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,30,0) * CFrame.Angles(0,math.rad(180),0))end
...
Attack.Kill2 = function(model,Succes)
  ...
  local Equipped = game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool")
  local ToolTip = Equipped.ToolTip
  if ToolTip == "Trái ác quỷ" then _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,10,0) * CFrame.Angles(0,math.rad(90),0)) else _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,30,8) * CFrame.Angles(0,math.rad(180),0))end
...
Attack.KillSea = function(model,Succes)
  ...
  local Equipped = game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool")
  local ToolTip = Equipped.ToolTip
  if ToolTip == "Trái ác quỷ" then _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,10,0) * CFrame.Angles(0,math.rad(90),0)) else notween(model.HumanoidRootPart.CFrame * CFrame.new(0,50,8)) wait(.85)notween(model.HumanoidRootPart.CFrame * CFrame.new(0,400,0)) wait(1)end
  end
end
...
Attack.Mas = function(model,Succes)
  ...
  BringEnemy()
    if model.Humanoid.Health <= HealthM then
      ...
      Useskills("Trái ác quỷ","Z")
      Useskills("Trái ác quỷ","X")
      Useskills("Trái ác quỷ","C")
    else
      weaponSc("Cận chiến")
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,30,0))
    end
...
statsSetings = function(Num, value)
  if Num == "Cận chiến" then
    if plr.Data.Points.Value ~= 0 then
      replicated.Remotes.CommF_:InvokeServer("AddPoint","Cận chiến",value)
    end
  elseif Num == "Defense" then
    if plr.Data.Points.Value ~= 0 then
      replicated.Remotes.CommF_:InvokeServer("AddPoint","Defense",value)
    end
  elseif Num == "Kiếm" then
    if plr.Data.Points.Value ~= 0 then
      replicated.Remotes.CommF_:InvokeServer("AddPoint","Kiếm",value)
    end
  elseif Num == "Súng" then
    if plr.Data.Points.Value ~= 0 then
      replicated.Remotes.CommF_:InvokeServer("AddPoint","Súng",value)
    end
...
Useskills = function(weapon, skill)
  ...
  elseif weapon == "Trái ác quỷ" then
    weaponSc("Trái ác quỷ")
    if skill == "Z" then
      vim1:SendKeyEvent(true, "Z", false, game);
      vim1:SendKeyEvent(false, "Z", false, game);
    elseif skill == "X" then
...
local Settings = {
    ["Tốc độ bay"] = 1.5, -- bay (duration = distance/(100*speed))
...
function GetSpawnPoint(x)
    local Spawns = workspace:FindFirstChild("_WorldOrigin") 
        ...
        and workspace._WorldOrigin.PlayerSpawns:FindFirstChild("Hải tặc")
...
_tp = function(target)
    ...
    local tweenInfo = TweenInfo.new(distance / math.max(50, (Settings["Tốc độ bay"] or 1.5) * 100), Enum.EasingStyle.Linear)
...
-- ========================================
-- MINIMIZE ICON (như nyann os v6)
-- rbxassetid://94678517792779
-- ========================================
do
  ...
  pcall(function()
    ...
    if pg then
      local o2 = pg:FindFirstChild("NyannMinimizeIcon")
  ...
  local function ToggleWithSpin()
    ...
    busy = true
    pcall(function()
      if RealWindow and RealWindow.Toggle then
        RealWindow:Toggle()
      end
    end)
    pcall(function()
      local goal = btn.Rotation + 360
...
local function MakeTabShim(zeoxTab)
  local current = nil
  local tab = {}

  local function ensureSection(name)
    name = tostring(name or "Tùy chọn")
    if not current then
      local ok, sec = pcall(function()
        return zeoxTab:CreateSection(name)
      end)
      if ok and sec then current = sec end
    end
    return current
  end
  ...
  function tab:AddDropdown(cfg)
    ...
    local cb = cfg.Callback or function() end
    pcall(function()
      if current.CreateDropdown then
        current:CreateDropdown({
          Name = name,
  ...
  function tab:AddSlider(cfg)
    ...
    local cb = cfg.Callback or function() end
    pcall(function()
      if current.CreateSlider then
        current:CreateSlider({
          Name = name,
          Description = cfg.Description or "",
          Min = minv,
          Max = maxv,
  ...
  function tab:AddParagraph(a, b)
    ...
    pcall(function()
      if current.CreateLabel then
        current:CreateLabel(title .. (desc ~= "" and (" — " .. desc) or ""))
      else
        current:CreateButton({
          Name = title,
          Description = desc,
  ...
  function tab:AddTextBox(cfg)
    ...
    local cb = cfg.Callback or function() end
    pcall(function()
      if current.CreateInput then
        current:CreateInput({
          Name = name,
          Description = cfg.Placeholder or cfg.Default or "",
  ...
  function tab:AddDiscordInvite(cfg)
    cfg = cfg or {}
    ensureSection("Social")
    pcall(function()
      current:CreateButton({
        Name = tostring(cfg.Title or "Discord"),
        Description = tostring(cfg.Description or cfg.Invite or ""),
        ButtonText = "Copy",
        Callback = function()
...
function Window:MakeTab(cfg)
  ...
  local page
  local ok, res = pcall(function()
    return RealWindow:CreateTab({ Name = title })
  end)
  if ok then page = res end
  if not page then
    warn("[nyann os] CreateTab fail:", title, res)
    return MakeTabShim({
...
function Window:Notify(opts)
  ...
  pcall(function()
    if RealWindow and RealWindow.Notify then
      RealWindow:Notify({
        Title = title,
        Content = msg,
...
local Tabs = {
    ...
    Main = Window:MakeTab({ Title = "Trang chủ", Icon = "rbxassetid://" }),
    ...
    Quests = Window:MakeTab({ Title = "Nhiệm vụ và vật phẩm", Icon = "rbxassetid://" }),
    SeaEvent = Window:MakeTab({ Title = "Sự kiện biển", Icon = "waves" }),
    Race = Window:MakeTab({ Title = "Tộc v4", Icon = "rbxassetid://" }),
    Prehistoric = Window:MakeTab({ Title = "Sự kiện đảo núi lửa", Icon = "" }),
    Esp = Window:MakeTab({ Title = "Chỉ số và hiển thị", Icon = "rbxassetid://" }),
    ...
    Shop = Window:MakeTab({ Title = "Cửu hàng", Icon = "rbxassetid://" }),
    Misc = Window:MakeTab({ Title = "Linh tinh", Icon = "rbxassetid://" })
}
...
Tabs.Font:AddSection("Phông chữ tùy chỉnh")
...
WeaponDropdown = Tabs.Main:AddDropdown({
    ...
    Callback = function(Value)
        ...
        if Value == nil or Value == "" or Value == "Không" then
            _G.ChooseWP = nil
            _G.SelectWeapon = nil
            return
        end
...
spawn(function()
    while task.wait(0.5) do
        pcall(function()
            if not _G.ChooseWP or _G.ChooseWP == "Không" then
                _G.SelectWeapon = nil
                return
            end
            if _G.ChooseWP == "Cận chiến" then
                for _,v in pairs(plr.Backpack:GetChildren()) do
                    if v.ToolTip == "Cận chiến" then
                        _G.SelectWeapon = v.Name
                    end
                end
            elseif _G.ChooseWP == "Kiếm" then
                for _,v in pairs(plr.Backpack:GetChildren()) do
                    if v.ToolTip == "Kiếm" then
                        _G.SelectWeapon = v.Name
                    end
                end
            elseif _G.ChooseWP == "Súng" then
                for _,v in pairs(plr.Backpack:GetChildren()) do
                    if v.ToolTip == "Súng" then
                        _G.SelectWeapon = v.Name
                    end
                end
            elseif _G.ChooseWP == "Trái ác quỷ" then
                for _,v in pairs(plr.Backpack:GetChildren()) do
...
Name = "Tự động cày vật chất kì dị", 
Description = "", 
Default = false,
Callback = function(Value)
...
Name = "Tự động dừng lại khi có vật phẩm lạ", 
Description = "", 
...
Tabs.Main:AddSection("Nhặt trái Berry")
...
Name = "Tự động nhặt trái Berry + Chuyển đổi máy chủ", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.AutoBerryH = Value
...
Tabs.Main:AddSection("Đánh quái ở tất cả đảo")
...
if World1 then
    Tabs.Main:AddDropdown({
        Name = "Chọn đảo (Biển thứ nhất)",
        Options = {"Hải tặc", "Marine", "Jungle", "Pirate Village", "Desert", "Frozen Village", "Marine Fortress", "Skylands Lower", "Prison", "Colosseum", "Magma Village", "Underwater City", "Skylands Upper"},
        Callback = function(Value)
...
Tabs.Main:AddSection("Cày Elite Hunter")
...
local EliteHunter = Tabs.Main:AddParagraph("Elite Spawn", "Trạng thái: ")
spawn(function()
...
Tabs.Main:AddSection("Đánh Rip Indra")
...
Tabs.Main:AddSection("Cày Hoàng tử bột")
...
Tabs.Main:AddToggle({
    Name = "Tự động đánh Bột đế vương [Fully]",
    Default = false,
    Callback = function(Value)
...
Tabs.Main:AddSection("Cày xương")
...
spawn(function()
    ...
    while wait(0.5) do
        ...
        pcall(function()
            ...
            local questUI =
                player.PlayerGui:FindFirstChild("Chính")
...
Name = "Tự động đánh Bạo chúa bầu trời", 
Description = "", 
Default = false,
Callback = function(Value) 
    _G.FarmTyrant = Value 
...
Name = "Tự động triệu hồi Bạo chúa bầu trời", 
Description = "", 
Default = false,
Callback = function(Value)
...
Tabs.Main:AddSection("Đánh trùm")
        ...
        if SelectIsland == "Cake" then
		  ...
		  if v then		      
		    ...
		    repeat wait()
              ...
              if plr.Character:FindFirstChildOfClass("Tool").ToolTip ~= "Súng" then return end
...
Name = "Tự động cày thông thạo cho tất cả kiếm", 
Description = "", 
Default = false,
Callback = function(Value)
...
Tabs.Settings:AddSection("Màu menu")
Tabs.Settings:AddDropdown({
  Name = "Chọn màu mà bạn muốn",
  Options = {
    "Tối",
    "Sáng",
    "Đỏ tối",
...
Tabs.Settings:AddSlider({
    Name = "Độ trễ khi chuyển đổi máy chủ",
    Min = 5,
    Max = 120,
    Default = 30,
    Increment = 1,
...
Name = "Tự động xoay quanh quái  xyz", 
Description = "", 
Default = true,
Callback = function(Value)
  RandomCFrame = Value
...
Tabs.Settings:AddToggle({
    Name = "Tự động chặn khi Admin tham gia máy chủ",
    Description = "",
    Default = true,
    Callback = function(Value)
...
Tabs.Esp:AddSection("Chỉ số thống kê")
...
spawn(function()
  while wait(Sec) do
    pcall(function()
    if _G.Auto_Melee then statsSetings("Cận chiến",pSats) end
    end)
  end
end)
...
spawn(function()
  while wait(Sec) do
    pcall(function()
    if _G.Auto_Sword then statsSetings("Kiếm",pSats) end
    end)
  end
end)
StatsUpg = Tabs.Esp:AddToggle({
...
spawn(function()
  while wait(Sec) do
    pcall(function()
    if _G.Auto_Gun then statsSetings("Súng",pSats) end
    end)
  end
end)
StatsUpg = Tabs.Esp:AddToggle({
Name = "Nâng vào trái ák quỷ", 
Description = "", 
Default = false,
Callback = function(Value)
...
BaitDropdown = Tabs.Fish:AddDropdown({
    Name = "Chọn mồi câu",
    Description = "",
    Options = {"Mồi cơ bản", "Kelp Bait", "Good Bait", "Abyssal Bait", "Frozen Bait", "Epic Bait", "Carnivore Bait"},
    Default = "Mồi cơ bản",
    Callback = function(Value)
...
Name = "Tự động làm nhiệm vụ biển thứ hai", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.TravelDres = Value
...
Name = "Tự động nhận nhiệm vụ biển thứ ba", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.AutoZou = Value
...
Tabs.Quests:AddToggle({
Name = "Tự farm Material Skull Guitar", 
Description = "", 
Default = false,
Callback = function(Value)
...
Q = Tabs.Quests:AddToggle({
Name = "Tự Teleport Barista Cousin", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.Tp_MasterA = Value
...
Q = Tabs.Quests:AddToggle({
Name = "Nhận nhiệm vụ Rainbow nhanh", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.GetQFast = Value
...
Tabs.Race:AddToggle({
    Name = "Tự Tween To Mirage Island",
    Description = "",
    Default = false,
    Callback = function(Value)
...
Tabs.Race:AddToggle({
Name = "Tự Tween To Highest Point", 
Description = "", 
Default = false,
Callback = function(Value)
...
Tabs.Race:AddToggle({
Name = "Tự Tween Advanced Fruit Dealer", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.Addealer = Value
...
Callback = function()
  ...
  replicated:WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("RaceV4Progress","Dịch chuyển")
  replicated:WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("RaceV4Progress","Continue")
end})
Tabs.Race:AddToggle({
Name = "Tự Look At Moon", 
Description = "", 
Default = false,
Callback = function(Value)
  LookM = Value
...
Tabs.Race:AddButton({
    Name = "Dịch chuyển to Temple of Time",
    Description = "",
    Callback = function()
...
Name = "Tự Teleport to Race Doors", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.TPDoor = Value
	    ...
	    if tostring(plr.Data.Race.Value) == "Fishman" then
	      if GetSeaBeastTrial() then            
            repeat task.wait()
              ...
              Useskills("Cận chiến","Z")
	          Useskills("Cận chiến","X")
	          Useskills("Cận chiến","C")
              wait(.1)
              Useskills("Kiếm","Z")
              Useskills("Kiếm","X")
              wait(.1)
...
Name = "Tự Kill Player After Trial", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.Defeating = Value
...
Toggle = Tabs.Prehistoric:AddToggle({
Name = "Tự Relic Drago Trial [Beta]", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.Relic123 = Value
...
Name = "Nâng Dragon Talon với Uzoth", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.DT_Uzoth = Value
...
Tabs.Prehistoric:AddToggle({
    Name = "Tự Find Prehistoric Island",
    Description = "",
    Default = false,
    Callback = function(Value)
...
Tabs.Prehistoric:AddToggle({
    Name = "Tự Start Prehistoric Event",
    Default = false,
    Callback = function(Value)
...
Tabs.Prehistoric:AddToggle({
    Name = "Tự Patch Prehistoric Event",
    Description = "",
    Default = false,
    Callback = function(Value)
                    ...
                    if glow and glow.Enabled then
                        repeat
                            ...
                            if plr:DistanceFromCharacter(layer.CFrame.Position) <= 150 then
                                ...
                                Useskills("Cận chiến","Z") wait(.4)
                                Useskills("Cận chiến","X") wait(.4)
                                Useskills("Cận chiến","C") wait(.4)
                                Useskills("Trái ác quỷ","Z") wait(.4)
                                Useskills("Trái ác quỷ","X") wait(.4)
                                Useskills("Trái ác quỷ","C")
                            end
...
Toggle = Tabs.Prehistoric:AddToggle({
Name = "Tự Reset When Complete Volcano", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.ResetPH = Value
...
Tabs.SeaEvent:AddToggle({
Name = "Tự Attack PirateGrandBrigade", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.PGB = Value
...
Tabs.SeaEvent:AddToggle({
Name = "Tự Teleport to Shrine Actived", 
Description = "", 
Default = false,
Callback = function(Value)
...
Tabs.SeaEvent:AddToggle({
Name = "Tự Teleport Frozen Dimension", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.FrozenTP = Value
...
DungeonTables = {"Flame","Ice","Quake","Sáng","Tối","String","Rumble","Magma","Human: Buddha","Sand","Bird: Phoenix","Dough"}
...
task.spawn(function()
    while task.wait(Sec) do
        ...
        pcall(function()
            ...
            local main = gui and gui:FindFirstChild("Chính")
...
Tabs.Combat:AddToggle({
    Name = "Dịch chuyển To Select Players",
    Description = "",
    Default = false,
    Callback = function(Value)
        _G.TpPly = Value
...
local HTTPService = game:GetService("HttpService")
local TeleportService = game:GetService("TeleportService")
local StatsService = game:GetService("Chỉ số")
...
task.spawn(function()
  while task.wait() do
    if _G.daylightN then
      if _G.SelectDN == "Ngày" then
        Lighting.ClockTime = 12
      elseif _G.SelectDN == "Đêm" then
...
function HitRegistrationModule.Execute()
    ...
    if #hitTargets > 0 and tool and (tool:GetAttribute("WeaponType") == "Cận chiến" or tool:GetAttribute("WeaponType") == "Kiếm") then
...
-- Fallback: Việt hóa chữ còn sót trên UI
task.defer(function()
  task.wait(0.25)
  local map = {
    ["Search..."]="Tìm...",["Tabs"]="Tab",["Select Bait"]="Chọn mồi",["Basic Bait"]="Mồi cơ bản",
    ["Auto Buy Bait"]="Tự mua mồi",["Auto Fishing"]="Tự câu cá",["Auto Quest Fishing"]="Tự nhiệm vụ câu",
    ["Auto Complete Quest"]="Tự hoàn thành nhiệm vụ",["Auto Sell Fish"]="Tự bán cá",
    ["Auto Spam Skill Z"]="Tự spam skill Z",["Version"]="Phiên bản",
  }
  local function go()
    ...
    for _,root in ipairs(roots) do
      for _,o in ipairs(root:GetDescendants()) do
        if (o:IsA("TextLabel") or o:IsA("TextButton") or o:IsA("TextBox")) and o.Text and map[o.Text] then
          o.Text = map[o.Text]
        end
      end
    end
