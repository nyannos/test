...
-- ========================================
-- FLUENTPLUS UI + shim (giữ Tabs.* farm API)
-- ========================================
print("[nyann os] loading FluentPlus...")

local Fluent
do
  local ok, res = pcall(function()
    return loadstring(game:HttpGet(
      "https://raw.githubusercontent.com/discoart/FluentPlus/refs/heads/main/release.lua"
    ))()
  end)
  if not ok or not res then
    error("[nyann os] FluentPlus fail: " .. tostring(res))
  end
  Fluent = res
  print("[nyann os] FluentPlus OK")
end
...
local RealWindow = Fluent:CreateWindow({
  Title = "nyann os library test (Beta 1.0)",
  SubTitle = " by real_@nyannnokonoko",
  TitleIcon = "rbxassetid://94678517792779",
...
-- Fluent tab shim → redzlib-style Tabs.*
local function MakeTabShim(fluentTab)
  local tab = {}
  local idCounter = 0
  local function nid(prefix)
    idCounter = idCounter + 1
    return (prefix or "el") .. "_" .. tostring(idCounter)
  end

  function tab:AddSection(name)
    pcall(function()
      fluentTab:AddParagraph({
  ...
  function tab:AddDropdown(cfg)
    ...
    if type(def) == "string" then
      for i, v in ipairs(opts) do
        if v == def then defIdx = i break end
      end
    ...
    local id = nid("drop")
    local ok, ret = pcall(function()
      return fluentTab:AddDropdown(id, {
        Title = name,
        Description = cfg.Description or "",
        Values = opts,
        Default = defIdx,
  ...
  function tab:AddParagraph(a, b)
    ...
    local content = (b ~= nil) and tostring(b) or ""
    local obj
    pcall(function()
      obj = fluentTab:AddParagraph({
        Title = title,
        Content = content,
      })
    end)
    local proxy = {
      Set = function(_, t)
        pcall(function()
          if obj and obj.SetTitle then obj:SetTitle(tostring(t or "")) end
        end)
      end,
      SetDesc = function(_, d)
        pcall(function()
          if obj and obj.SetDesc then obj:SetDesc(tostring(d or ""))
          elseif obj and obj.SetContent then obj:SetContent(tostring(d or ""))
          end
        end)
      end,
  ...
  function tab:AddTextBox(cfg)
    cfg = cfg or {}
    local name = cfg.Name or cfg.Title or "TextBox"
    local id = nid("inp")
    local ok, ret = pcall(function()
      return fluentTab:AddInput(id, {
        Title = name,
  ...
  function tab:AddDiscordInvite(cfg)
    ...
    pcall(function()
      fluentTab:AddParagraph({
        Title = tostring(cfg.Title or cfg.Name or "Discord"),
        Content = tostring(cfg.Invite or cfg.Description or ""),
      })
    end)
  end
...
function Window:MakeTab(cfg)
  ...
  local icon = cfg.Icon
  if type(icon) == "string" and (icon == "" or icon == "rbxassetid://") then
    icon = nil
  end
  local page
  local ok, res = pcall(function()
    return RealWindow:AddTab({
  ...
  if not page then
    warn("[nyann os] Fluent Tab fail:", title, res)
    return MakeTabShim({ AddParagraph = function() end, AddToggle = function() end })
  end
...
function Window:Notify(opts)
  ...
  pcall(function()
    Fluent:Notify({
      ...
      Content = tostring(opts.Content or opts.Message or ""),
...
function Window:SelectTab(i)
  pcall(function() RealWindow:SelectTab(i) end)
end

-- hiện tab đầu sau khi build
task.defer(function()
  task.wait(0.4)
  pcall(function() RealWindow:SelectTab(1) end)
end)


print("[nyann os] FluentPlus menu ready")

-- ========================================
-- MINIMIZE ICON (Fluent) - hiện rõ, kéo được
...
do
  ...
  local function setFluentVisible(vis)
    ...
    pcall(function()
      if RealWindow and RealWindow.Minimize then
        -- chỉ gọi khi trạng thái lệch
        local hidden = false
        pcall(function()
          if RealWindow.Root then
            hidden = not RealWindow.Root.Visible
          end
        end)
        if MenuVisible == hidden then
          RealWindow:Minimize()
        end
      end
    end)
    -- fallback: bật/tắt ScreenGui Fluent
    pcall(function()
      local pg = Players.LocalPlayer:FindFirstChild("PlayerGui")
      for _, parent in ipairs({ CoreGui, pg }) do
        ...
        for _, g in ipairs(parent:GetChildren()) do
          if g:IsA("ScreenGui") then
            ...
            if n:find("fluent") or n:find("library") then
              -- không tắt hẳn GUI (mất minimize), chỉ ẩn frame chính
            end
          end
        end
      end
    ...
    pcall(function()
      if RealWindow and RealWindow.Root then
        RealWindow.Root.Visible = MenuVisible
      end
    end)
  end

  local function toggleMenu()
    setFluentVisible(not MenuVisible)
  end
  ...
  btn.Position = UDim2.new(0, 16, 0.35, 0)
  ...
  corner.CornerRadius = UDim.new(0, 10) -- vuông bo nhẹ
  corner.Parent = btn
  ...
  -- Click = toggle (bỏ qua nếu kéo xa)
  local clickPos
  ...
  btn.MouseButton1Click:Connect(function()
    ...
    -- xoay nhẹ khi bấm
    pcall(function()
      local tw = TS:Create(btn, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
  ...
  print("[nyann os] Minimize icon visible @ top-left")
end
