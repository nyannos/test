...
-- Language is intentionally placed in Miscellaneous.
v15:AddDropdown({
    Name = "Language",
    ...
    Callback = function(value)
        if value == "Vietnamese" then
            -- Translation hook. Add translated UI strings here without
            -- modifying the existing script functions.
            pcall(function()
                local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
                if not pg then return end
                for _, obj in ipairs(pg:GetDescendants()) do
                    if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                        if obj.Text == "Miscellaneous" then obj.Text = "Khác" end
                        if obj.Text == "Shopping" then obj.Text = "Mua sắm" end
                        if obj.Text == "Settings Farm" then obj.Text = "Cài đặt Farm" end
                    end
                end
            end)
        end
...
local LanguageMap = {
    Vietnamese = {
        ...
        ["Status/Server"] = "Trạng thái/Máy chủ",
        ["Farming"] = "Farm",
        ["Quests & Other"] = "Nhiệm vụ & Khác",
        ["Settings Farm"] = "Cài đặt Farm",
        ["Auto Fishing"] = "Tự động câu cá",
        ["Sea Event"] = "Sự kiện biển",
        ["Volcano & Dojo"] = "Núi lửa & Dojo",
        ["Mirage & Race"] = "Mirage & Race",
        ["Fruits & Check Stock"] = "Trái cây & Kiểm tra kho",
        ["Raid & Dungoen"] = "Raid & Hầm ngục",
        ["Teleport"] = "Dịch chuyển",
        ["PvP & Player"] = "PvP & Người chơi",
        ["Esp & Stats"] = "ESP & Chỉ số",
        ["Shopping"] = "Mua sắm",
        ["Miscellaneous"] = "Khác",
        ["Select Attack Mode"] = "Chọn chế độ đánh",
        ["Normal Attack"] = "Đánh thường",
        ["Fast Attack"] = "Đánh nhanh",
        ["Super Attack"] = "Đánh siêu nhanh",
        ["nyann Attack"] = "nyann Attack",
        ["Super nyann Attack"] = "Super nyann Attack",
        ["Enable Fast Attack"] = "Bật Fast Attack",
        ["Enable Auto Click"] = "Bật Auto Click",
        ["Select target type"] = "Chọn loại mục tiêu",
        ["Gun range"] = "Tầm súng",
        ["Shoot Gun Aura ( Dùng Dragon Strorm)"] = "Bắn Gun Aura (Dùng Dragon Storm)",
        ["Mirage Island"] = "Đảo Mirage",
        ["Status: "] = "Trạng thái: ",
        ["Settings"] = "Cài đặt",
        ["Language"] = "Ngôn ngữ",
        ["English"] = "Tiếng Anh",
...
local function TranslateText(value)
    if type(value) ~= "string" then return value end
    if Language == "English" then return value end

    local dict = LanguageMap[Language]
    if dict and dict[value] then
        return dict[value]
    end
...
local function TranslateGui()
    ...
    for _, obj in ipairs(gui:GetDescendants()) do
        if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
            if OriginalText[obj] == nil then
                OriginalText[obj] = obj.Text
            end

            local original = OriginalText[obj]
            obj.Text = TranslateText(original)
        end
    end
end

-- Tự động áp dụng bản dịch khi GUI thay đổi.
local TranslationRunning = true
task.spawn(function()
    while TranslationRunning do
        pcall(TranslateGui)
        task.wait(1)
    end
end)

-- Chọn ngôn ngữ nằm trong phần Settings Farm.
v4:AddDropdown({
    Name = "Language",
    Options = {"English", "Vietnamese"},
    Default = "English",
