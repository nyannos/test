...
local function UpdatePlayerESP()
    for _, player in pairs(game:GetService("Người chơi"):GetPlayers()) do
        pcall(function()
            -- Bỏ qua bản thân
            ...
            if PlayerESPEnabled then
                ...
                if not head:FindFirstChild("Người chơiESP" .. PlayerESPNumber) then
                    local billboard = Instance.new("BillboardGui")
                    billboard.Name = "Người chơiESP" .. PlayerESPNumber
                ...
                -- Cập nhật nội dung
                local label = head["Người chơiESP" .. PlayerESPNumber].TextLabel
                ...
                -- Xóa ESP nếu tồn tại
                local esp = head:FindFirstChild("Người chơiESP" .. PlayerESPNumber)
            ...
            for _, player in pairs(game.Players:GetPlayers()) do
                pcall(function()
                    if player.Character and player.Character:FindFirstChild("Head") then
                        local esp = player.Character.Head:FindFirstChild("Người chơiESP" .. PlayerESPNumber)
                        if esp then esp:Destroy() end
                    end
                end)
            end
        ...
        if _G.ChestBypass then  -- dùng biến _G.ChestBypass
            ...
            while _G.ChestBypass and (tick() - startTick) < 4 do
                ...
                for i = 1, #chests do
                    ...
                    if not chest:GetAttribute("IsĐã tắt") then
...
repeat
    local loading = plr.PlayerGui:FindFirstChild("Chính")
    loading = loading and loading:FindFirstChild("Đang tải")
    task.wait()
		...
		if tostring(I) == "FireServer" then
			if tostring(e[1]) == "RemoteEvent" then
				if tostring(e[2]) ~= "true" and tostring(e[2]) ~= "false" then
					if _G.FarmMastery_G and not SoulGuitar or _G.FarmMastery_Dev or _G.FarmBlazeEM or _G.Prehis_Skills or _G.SeaBeast1 or _G.FishBoat or _G.PGB or _G.Leviathan1 or _G.Complete_Trials or _G.AimMethod and ABmethod == "AimBots Skill" or _G.AimMethod and ABmethod == "Tự động ngắm" then
						e[2] = MousePos;
						return i(unpack(e));
					end;
				end;
...
CheckEnemiesBoat = function()
		for I, e in pairs(workspace.Enemies:GetChildren()) do
			if e.Name == "FishBoat" and (e:FindFirstChild("Máu")).Value > 0 then
				return true;
			end;
		end;
		return false;
...
CheckPirateGrandBrigade = function()
		for I, e in pairs(workspace.Enemies:GetChildren()) do
			if (e.Name == "PirateGrandBrigade" or e.Name == "PirateBrigade") and (e:FindFirstChild("Máu")).Value > 0 then
				return true;
			end;
		end;
		return false;
...
_G.Settings.FastAttackMode = "Đánh nhanh"
...
local TargetTypes = {"Người chơi", "NPC", "All"}
...
local function findClosestTargetForGun()
    ...
    if targetType == "Người chơi" or targetType == "All" then
    ...
    pcall(function()
        while wait() do
            if game:GetService("Workspace").Enemies:FindFirstChild("Tyrant of the Skies") then
                TyrantStatus:SetDesc("Trạng thái : ✅️")
            else
                TyrantStatus:SetDesc("Trạng thái : ❌️")
            end
        end
    ...
    while wait() do
        pcall(function()
            ...
            or game:GetService("Workspace").Enemies:FindFirstChild("Dough King") then
                CheckDoughKing:SetDesc("Trạng thái : ✅️")
            else
                CheckDoughKing:SetDesc("Trạng thái : ❌️")
            end
        end)
    ...
    while wait() do
        pcall(function()
            ...
            or ws:FindFirstChild("Diablo") or ws:FindFirstChild("Deandre") or ws:FindFirstChild("Urban") then
                EliteHunter:SetDesc("Trạng thái : ✅️ | Killed: " .. progress)
            else
                EliteHunter:SetDesc("Trạng thái : ❌️ | Killed: " .. progress)
            end
        end)
...
spawn(function()
    while wait() do
        if getgenv().Join then
            game:GetService("Dịch chuyểnService"):TeleportToPlaceInstance(game.PlaceId,getgenv().Job,game.Players.LocalPlayer)
        end
    end
end)
v1:AddButton({
...
v1:AddButton({
    ...
    Callback = function()
        game:GetService("Dịch chuyểnService"):Teleport(game.PlaceId,game:GetService("Người chơi").LocalPlayer)
    end
})
v1:AddButton({
    Name = "Đổi máy chủ",
...
v1:AddButton({
    ...
    Callback = function()
        ...
        local TPS = game:GetService("Dịch chuyểnService")
        local Players = game:GetService("Người chơi")
...
v4:AddDropdown({
    Name = "Chọn chế độ tấn công",
    Options = {"Normal Attack", "Đánh nhanh", "Super Attack", "Ultra Attack", "Ultimte Attack"},
    Default = "Đánh nhanh",
    Multi = false,
...
v4:AddToggle({
    Name = "Tự động click khi cầm súng (2 giây)",
    Default = false,
    Callback = function(val)
...
v2:AddSection({"Tự động Farm chính"})
v2:AddDropdown({
    Name = "Chọn vũ khí",
...
v2:AddToggle({
    Name = "Bắt đầu Farm",
    Default = GetSetting("Bắt đầuFarm_Save", false),
    Callback = function(v)
        _G.StartFarm = v
        ...
        _G.SaveData["Bắt đầuFarm_Save"] = v
        SaveSettings()
    end
})

v2:AddToggle({
    Name = "Accept Nhiệm vụs",
    Default = GetSetting("AcceptNhiệm vụ_Save", false),
    Callback = function(v)
        _G.AcceptQuest = v
        _G.SaveData["AcceptNhiệm vụ_Save"] = v
        SaveSettings()
    end
})
...
if myLevel >= 2575 then
    return {
        Name = "Skull Slayer",
        QuestArgs = {"Bắt đầuNhiệm vụ", "TikiNhiệm vụ3", 2},
        QuestPos = CFrame.new(
...
elseif myLevel > 2550 then
    return {
        Name = "Serpent Hunter",
        QuestArgs = {"Bắt đầuNhiệm vụ", "TikiNhiệm vụ3", 1},
        QuestPos = CFrame.new(
            -16665.0879, 105.27478, 1577.61743,
            -0.999621451, 3.5280582e-08, 0.0275127869,
             3.5990368e-08, 1, 2.53032191e-08,
...
elseif myLevel >= 2525 then
    return {
        Name = "Isle Champion",
        QuestArgs = {"Bắt đầuNhiệm vụ", "TikiNhiệm vụ2", 2},
        QuestPos = CFrame.new(
...
elseif myLevel >= 2500 then
    return {
        ...
        QuestArgs = {"Bắt đầuNhiệm vụ", "TikiNhiệm vụ2", 1},
        QuestPos = CFrame.new(
            -16546.748, 55.7216759, -172.865311,
            -0.0595058464, 2.45485676e-08, 0.998227954,
            -5.3272025e-08, 1, -2.77677703e-08,
...
elseif myLevel >= 2475 then
    return {
        Name = "Island Boy",
        QuestArgs = {"Bắt đầuNhiệm vụ", "TikiNhiệm vụ1", 2},
        QuestPos = CFrame.new(
            -16546.748, 55.7216759, -172.865311,
            -0.0595058464, 2.45485676e-08, 0.998227954,
            -5.3272025e-08, 1, -2.77677703e-08,
...
-- [LEVEL < 2475] ISLE OUTLAW
    ...
    return {
        Name = "Isle Outlaw",
        QuestArgs = {"Bắt đầuNhiệm vụ", "TikiNhiệm vụ1", 1},
        QuestPos = CFrame.new(
            -16546.748, 55.7216759, -172.865311,
            -0.0595058464, 2.45485676e-08, 0.998227954,
            -5.3272025e-08, 1, -2.77677703e-08,
            ...
            pcall(function()
                ...
                if _G.AcceptQuest and not QuestUI.Visible then      
                    ...
                    if distToNPC > 5 then
                        ...
                        task.wait(0.5)
                        local quests = {
                            {"Bắt đầuNhiệm vụ","HauntedNhiệm vụ1",1}, 
                            {"Bắt đầuNhiệm vụ","HauntedNhiệm vụ1",2},
                            {"Bắt đầuNhiệm vụ","HauntedNhiệm vụ2",1}, 
                            {"Bắt đầuNhiệm vụ","HauntedNhiệm vụ2",2}
                        }
    ...
    while task.wait() do
        if _G.Level and _G.StartFarm then
            pcall(function()
                ...
                local Q = QuestNeta()  
                if not string.find(QuestTitle, Q[5]) then game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonNhiệm vụ") end      
                if not plr.PlayerGui.Main.Quest.Visible then      
                    ...
                    if (Root.Position - Q[6].Position).Magnitude <= 50 then      
                        task.wait(1.9)  
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Bắt đầuNhiệm vụ", Q[3], Q[2])      
                    end      
                    return      
                end      
            ...
            pcall(function()
                ...
                if _G.AcceptQuest and not plr.PlayerGui.Main.Quest.Visible then  
                    ...
                    if (hrp.Position - questPos.Position).Magnitude <= 40 then  
                        local q = {{"Bắt đầuNhiệm vụ", "CakeNhiệm vụ2", 2}, {"Bắt đầuNhiệm vụ", "CakeNhiệm vụ2", 1}, {"Bắt đầuNhiệm vụ", "CakeNhiệm vụ1", 1}, {"Bắt đầuNhiệm vụ", "CakeNhiệm vụ1", 2}}  
...
spawn(function()
	while wait(Sec) do
		if _G.AutoBerry then
			...
			local e = game:GetService("Người chơi");
            ...
            pcall(function()
                ...
                for _, chest in pairs(Chests) do
                    ...
                    if not SelectedIsland or chest:IsDescendantOf(SelectedIsland) then
                        if not chest:GetAttribute("IsĐã tắt") and dist < minDist then
                            minDist = dist
                            nearestChest = chest
                        end
                    end
        ...
        pcall(function()
            if _G.FindBoss and (game:GetService("ReplicatedStorage"):FindFirstChild(_G.FindBoss) or 
               ...
               workspace:FindFirstChild(_G.FindBoss)) then
                BossStatus:SetDesc("✅ Boss đã xuất hiện")
            else
                BossStatus:SetDesc("❌ Không tìm thấy Boss")
            end
        end)
    end
end)
...
v2:AddToggle({
    ...
    Default = GetSetting("AutoAcceptNhiệm vụBoss_Save", false),
    Callback = function(v)
        _G.AutoAcceptQuest = v
        _G.SaveData["AutoAcceptNhiệm vụBoss_Save"] = v
        SaveSettings()
    end
})
        ...
        local function FastScan(container, isRS)
            for _, v in pairs(container:GetDescendants()) do
                if v:IsA("Model") and v:GetAttribute("IsBoss") == true then
                    if v.Name ~= "Boss Bộ xương bị nguyền" then
        ...
        pcall(function()
            if _G.AutoBoss and _G.FindBoss then
                ...
                if _G.AutoAcceptQuest and Qname and Qdata and PosQBoss then
                    local playerGui = game.Players.LocalPlayer.PlayerGui:FindFirstChild("Chính") and game.Players.LocalPlayer.PlayerGui.Main:FindFirstChild("Nhiệm vụ")
                ...
                if NeedQuest then
                    if (PosQBoss.Position - char.HumanoidRootPart.Position).Magnitude <= 5 then
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Bắt đầuNhiệm vụ", Qname, Qdata)
                    else
...
v2:AddDropdown({
    Name = "Chọn đảo để Farm thông thạo",
    Options = {"Cake", "Bone"},
...
-- ========================================
v2:AddToggle({
    Name = "Tự động Farm thông thạo Trái ác quỷ",
...
v2:AddToggle({
    Name = "Tự động Farm thông thạo tất cả Kiếm",
                    ...
                    if type(item) == "table" and item.Type == "Sword" then
                        ...
                        if tonumber(item.Mastery) >= 1 and tonumber(item.Mastery) <= 599 then
                            if GetBP(swordName) then
                                ...
                                replicated.Remotes.CommF_:InvokeServer("LoadVật phẩm", swordName)
                            end
        ...
        if data and data.data then
            for _, server in ipairs(data.data) do
                if server.playing < server.maxPlayers and server.id ~= game.JobId then
                    game:GetService("Dịch chuyểnService"):TeleportToPlaceInstance(game.PlaceId, server.id, game:GetService("Người chơi").LocalPlayer)
                    return true
                end
            end
        end
...
v4:AddToggle({
    Name = "Tự động đổi máy chủ khi Admin vào",
...
spawn(function()
    local player = game:GetService("Người chơi").LocalPlayer
    local runService = game:GetService("RunService")
...
v4:AddSection({"Tự động Hop"})

v4:AddToggle({
    Name = "Tự động đổi máy chủ",
...
local FishingClientConfig = require(FishReplicated:WaitForChild("FishingClient"):WaitForChild("Cấu hình"))
            ...
            pcall(function()
                ...
                if equippedTool then
                    ...
                    if targetPos and (state == "ReeledIn" or serverState == "ReeledIn") then
                        FishingRequest:InvokeServer("Bắt đầuCasting")
                        task.wait()
                    ...
                    elseif serverState == "Biting" then
                        ...
                        redzlib:Notify({
                            Title = "Tự động câu cá",
                            Message = "Đã bắt được vật phẩm mới",
                            Duration = 3
                        })
                    end
                end
                    ...
                    if plr.Data.Level.Value >= 700 then
                        ...
                        if IceDoor then
                            ...
                            if IceDoor.CanCollide == true and IceDoor.Transparency == 0 then
                                ...
                                replicated.Remotes.CommF_:InvokeServer("DressrosaNhiệm vụProgress", "Detective")
                                EquipWeapon("Key")
                                repeat
                                    wait()
            ...
            pcall(function()
                if _G.AutoZou then
                    ...
                    if plr.Data.Level.Value >= 1500 then
                        local bartiloProgress = replicated.Remotes.CommF_:InvokeServer("BartiloNhiệm vụProgress", "Bartilo")
                        ...
                        if bartiloProgress == 0 then
                            ...
                            if not QuestUI.Visible then
                                ...
                                if (Root.Position - CFrame.new(-456.28952, 73.0200958, 299.895966).Position).Magnitude <= 5 then
                                    wait(1)
                                    replicated.Remotes.CommF_:InvokeServer("Bắt đầuNhiệm vụ", "BartiloNhiệm vụ", 1)
                                end
                            else
...
v5:AddToggle({
    Name = "Tự động nhiệm vụ câu cá",
    Default = GetSetting("Fish_AutoNhiệm vụ", false),
    Callback = function(Value)
        _G.AutoFishingQuest = Value
        _G.SaveData["Fish_AutoNhiệm vụ"] = Value
        SaveSettings()
    end
})

local function HasQuest()
    local playerGui = Players.LocalPlayer:FindFirstChild("Người chơiGui")
    if playerGui then
        local questGui = playerGui:FindFirstChild("Nhiệm vụ") or playerGui:FindFirstChild("Nhiệm vụGui")
        if questGui and questGui:FindFirstChild("Container") and questGui.Container:FindFirstChild("Nhiệm vụTitle") then
            return true
        end
    end
    return false
        ...
        if _G.AutoFishingQuest then
            pcall(function()
                if not HasQuest() then
                    JobsRemote:InvokeServer("FishingNPC", "Angler", "AskNhiệm vụ")
                end
            end)
        end
    end
...
v5:AddToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            pcall(function()
                JobsRemote:InvokeServer("FishingNPC", "FinishNhiệm vụ")
            end)
        end
    end
})
...
-- 8. AUTO SPAM SKILL Z
v5:AddToggle({
    Name = "Tự động dùng kỹ năng của cần câu",
...
if World3 then
            ...
            if _G.AutoKenVTWO then
                pcall(function()
                    ...
                    elseif plr.PlayerGui.Main.Quest.Visible == false then
                        replicated.Remotes.CommF_:InvokeServer("CitizenNhiệm vụProgress", "Citizen");
                        wait(.1);
                        replicated.Remotes.CommF_:InvokeServer("Bắt đầuNhiệm vụ", "CitizenNhiệm vụ", 1);
                    end;
                    if replicated.Remotes.CommF_:InvokeServer("CitizenNhiệm vụProgress", "Citizen") == 2 then
                    ...
                    if not plr.Backpack:FindFirstChild("Fruit Bowl") or not plr.Character:FindFirstChild("Fruit Bowl") then
                        ...
                        if plr.Backpack:FindFirstChild("Fruit Bowl") or plr.Character:FindFirstChild("Fruit Bowl") then
                            ...
                            elseif plr.Character.HumanoidRootPart.CFrame == K then
                                replicated.Remotes.CommF_:InvokeServer("KenTalk2", "Bắt đầu");
                                wait(.1);
    ...
    -- AUTO CITIZEN QUEST
    v3:AddToggle({
        Name = "Tự động nhiệm vụ Công dân",
        Default = false,
        Callback = function(I)
            _G.CitizenQuest = I;
        end,
        ...
        while wait(Sec) do
            pcall(function()
                if _G.CitizenQuest then
                    if Lv >= 1800 and (replicated.Remotes.CommF_:InvokeServer("CitizenNhiệm vụProgress")).KilledBandits == false then
                    ...
                    elseif Lv >= 1800 and (replicated.Remotes.CommF_:InvokeServer("CitizenNhiệm vụProgress")).KilledBoss == false then
                        local I = GetConnectionEnemies("Captain Elephant");
                    ...
                    elseif Lv >= 1800 and replicated.Remotes.CommF_:InvokeServer("CitizenNhiệm vụProgress", "Citizen") == 2 then
    ...
    v3:AddToggle({
        Name = "Tự động nhiệm vụ Elite",
        Default = GetSetting("AutoEliteNhiệm vụ_Save", false),
        Callback = function(I)
            _G.FarmEliteHunt = I
            _G.SaveData["AutoEliteNhiệm vụ_Save"] = I
            SaveSettings()
        end,
    })
    ...
    v3:AddToggle({
        Name = "Dừng khi nhận được Cốc Thánh",
...
-- ========================================
v3:AddToggle({
    Name = "Nhận nhiệm vụ Bypass [Nguy hiểm]",
    Default = GetSetting("BypassNhiệm vụ_Save", false),
    Callback = function(I)
        _G.GetQFast = I
        _G.SaveData["BypassNhiệm vụ_Save"] = I
        SaveSettings()
    end,
})

v3:AddSection({"Vật phẩm"})
...
v3:AddToggle({
    Name = "Tự động Chìa khóa Rengoku",
...
-- ========================================
-- AUTO POLE V2 logic (toggle UI nằm ở "Vật phẩm chung")
        ...
        pcall(function()
            if _G.AutoPoleV2 then
                if not GetBP("Pole (1st Form)") then
                    replicated.Remotes.CommF_:InvokeServer("LoadVật phẩm", "Pole (1st Form)")
                end
                if not GetBP("Pole (2nd Form)") then
                    replicated.Remotes.CommF_:InvokeServer("LoadVật phẩm", "Pole (2nd Form)")
                end
            ...
            if _G.Greybeard then
                pcall(function()
                    if not GetWP("Bisento") then
                        replicated.Remotes.CommF_:InvokeServer("BuyVật phẩm", "Bisento")
        ...
        while task.wait(1) do
            if _G.Auto_Soul_Guitar then
                ...
                if questStatus then
                    ...
                    if questStatus.Swamp == false then status = "Nhiệm vụ 1: Đầm lầy"
                    elseif questStatus.Gravestones == false then status = "Nhiệm vụ 2: Bia mộ"
                    elseif questStatus.Ghost == false then status = "Nhiệm vụ 3: Ma"
                    elseif questStatus.Trophies == false then status = "Nhiệm vụ 4: Cúp"
                    elseif questStatus.Pipes == false then status = "Nhiệm vụ 5: Ống"
                    else status = "Final Step/Complete" end
                    SoulGuitarStatus:SetDesc(status)
                end
            else
                SoulGuitarStatus:SetDesc("Đã tắt")
            end
        end
    end)
end
...
if World3 then
    ...
    local CDKProgress = v3:AddParagraph({
        Title = "CDK Progress",
        Desc = "Kiểm tra tiến độ nhiệm vụ..."
    })

    task.spawn(function()
        while task.wait(2) do
            pcall(function()
                local progress = replicated.Remotes.CommF_:InvokeServer("CDKNhiệm vụ", "Progress")
    ...
    v3:AddToggle({
        Name = "Tự động hạ Boss Bộ xương bị nguyền",
    ...
    spawn(function()
        while wait(Sec) do
            pcall(function()
                if _G.CDK then
                    local boss = GetConnectionEnemies("Boss Bộ xương bị nguyền")
                    if boss then
                        repeat
                            wait()
...
-- ========================================
v3:AddSection({"Tam kiếm chân chính"})
...
v3:AddToggle({
    Name = "Dịch chuyển đến Người bán Kiếm Huyền thoại",
...
if World3 then
    v3:AddToggle({
        Name = "Tự động Bigmom (Nữ hoàng Bánh)",
...
if World3 then
    v3:AddToggle({
        Name = "Tự động Hình nộm luyện tập",
...
v6:AddButton({
    ...
    Callback = function()
        if _G.SelectedBoat then
            ...
            redzlib:Notify({
                Title = "Boat Shop",
                Message = "Vui lòng chọn thuyền trước",
                Duration = 3
            })
        end
    end
...
v6:AddSection({"Chọn thứ bạn muốn Farm"})

v6:AddToggle({
        Name = "Tự động tấn công Quái biển",
        Default = false,
        Callback = function(Value)
            _G.SeaBeast1 = Value
        end
...
v6:AddToggle({
    Name = "Tự động tấn công Pirate GrandBrigade",
    Default = false,
    Callback = function(Value)
        _G.PGB = Value
    end
...
if World1 then
    v6:AddSection({"Đi đến Biển 3 hoặc Biển 2 để Farm sự kiện hàng hải"})
end
if game.PlaceId == 7449423635 or game.PlaceId == 100117331123089 then
    ...
    v6:AddToggle({
        Name = "Tự động Cá mập kinh hoàng",
        Default = false,
        Callback = function(Value)
    ...
    v6:AddToggle({
        Name = "Tự động hạ thành viên Fish Crew",
        Default = false,
        Callback = function(Value)
            _G.MobCrew = Value
        end
    ...
    v6:AddToggle({
        Name = "Tự động hạ thành viên Haunted Crew",
        Default = false,
        Callback = function(Value)
            _G.HCM = Value
        end
    ...
    v6:AddToggle({
        Name = "Tự động tấn công thuyền cá",
        Default = false,
        Callback = function(Value)
            _G.FishBoat = Value
        end
...
if game.PlaceId == 7449423635 or game.PlaceId == 100117331123089 then
    ...
    v6:AddToggle({
        Name = "Dịch chuyển đến Không gian Băng giá",
    ...
    v6:AddToggle({
        Name = "Tự động dịch chuyển đến Đền khi kích hoạt",
        Default = false,
        Callback = function(Value)
...
-- ========================================
v7:AddSection({"Tùy chọn Tiền sử chính"})
...
v7:AddToggle({
    ...
    Default = GetSetting("AutoBắt đầuPrehistoric_Save", false),
    Callback = function(Value)
        _G.AutoStartPrehistoric = Value
        _G.SaveData["AutoBắt đầuPrehistoric_Save"] = Value
        SaveSettings()
    end
})
...
-- Auto Patch (Xóa lava)
v7:AddToggle({
    Name = "Tự động sửa lỗi (Xóa dung nham)",
...
task.spawn(function()
    while task.wait(1) do
        if _G.KillAura then
            ...
            KillAuraDisplay:SetDesc("Đã hạ: 0 | Đã tắt")
        end
    end
end)
...
v7:AddToggle({
    Name = "Tự động hạ Golem dung nham",
        ...
        pcall(function()
            if _G.Prehis_Find then
                ...
                if not PrehistoricLoc then
                    ...
                    local TeleportPart = PrehistoricLoc:FindFirstChild("HeadDịch chuyển", true) or PrehistoricLoc:FindFirstChild("Dịch chuyển_Head", true) or PrehistoricLoc:FindFirstChild("Head", true)
    ...
    while wait(0.3) do
        if _G.Prehis_Skills then
            pcall(function()
                ...
                local TrialTeleport = PrehistoricMap:FindFirstChild("TrialDịch chuyển")
                for _, obj in pairs(PrehistoricMap:GetDescendants()) do
...
v7:AddButton({
    Name = "Chế tạo Nam châm Núi lửa (Thủ công)",
    Callback = function()
        pcall(function()
...
v7:AddToggle({
    Name = "Tự động chế tạo Nam châm Núi lửa",
...
-- Auto Drago V1
v7:AddToggle({
    Name = "Tự động Drago (V1) - Lấy Trứng Rồng",
...
-- Auto Drago V3
v7:AddToggle({
    Name = "Tự động Drago (V3) - Cá mập kinh hoàng/Quái biển",
...
v7:AddToggle({
    Name = "Tự động Thử thách Di vật Drago [Beta]",
...
-- ========================================
v7:AddSection({"Nhiệm vụ Võ đường"})

-- Teleport to Dragon Dojo
v7:AddButton({
    Name = "Dịch chuyển đến Võ đường Rồng",
    Callback = function()
        pcall(function()
...
-- Auto Dojo Trainer
v7:AddToggle({
    Name = "Tự động Huấn luyện viên Võ đường (Hoàn thành nhiệm vụ)",
...
v7:AddToggle({
    Name = "Tự động Săn Rồng (Hydra/Venomous)",
        ...
        if _G.UPGDrago then
            pcall(function()
                ...
                if (DojoPos.Position - Root.Position).Magnitude >= 300 then
                    ...
                    replicated.Modules.Net:FindFirstChild("RF/InteractDragonNhiệm vụ"):InvokeServer(unpack(args))
                end
            end)
        end
    end
        ...
        if _G.Relic123 then
            pcall(function()
                if workspace.Map:FindFirstChild("DracoTrial") then
                    ...
                    local TrialTeleport = workspace.Map.PrehistoricIsland:FindFirstChild("TrialDịch chuyển")
        ...
        if _G.DT_Uzoth then
            pcall(function()
                ...
                if (UzothPos.Position - Root.Position).Magnitude <= 25 then
                    ...
                    replicated.Modules.Net:FindFirstChild("RF/InteractDragonNhiệm vụ"):InvokeServer(args)
                end
            end)
        end
    end
...
-- ========================================
v8:AddSection({"Tộc V4 / Đền Thời gian"})

-- Teleport to Temple of Time
v8:AddButton({
    Name = "Dịch chuyển đến Đền Thời gian",
    Callback = function()
        pcall(function()
            ...
            if not workspace.Map:FindFirstChild("Đền Thời gian") and World3 then
                local MapStash = replicated:FindFirstChild("MapStash")
                if MapStash and MapStash:FindFirstChild("Đền Thời gian") then
                    MapStash["Đền Thời gian"].Parent = workspace.Map
                end
            end
        end)
    end
...
v8:AddButton({
    Name = "Dịch chuyển đến Đồng hồ Cổ đại",
    Callback = function()
        pcall(function()
            local TemplePos = CFrame.new(28286.35546875, 14895.301757812, 102.62469482422)
...
v8:AddButton({
    ...
    Callback = function()
        pcall(function()
            ...
            replicated.Remotes.CommF_:InvokeServer("RaceV4Progress", "Dịch chuyển")
...
-- RACE TRIALS
-- ========================================
v8:AddSection({"Nhiệm vụ Thử thách V4"})
...
v8:AddToggle({
    Name = "Tự động hạ người chơi sau Thử thách",
...
spawn(function()
    while wait(Sec) do
        if _G.Lver then
            pcall(function()
                local Temple = workspace.Map:FindFirstChild("Đền Thời gian")
                if Temple then
...
local function GetSeaBeastTrial()
    ...
    if TrialLoc then
        for _, beast in pairs(workspace.SeaBeasts:GetChildren()) do
            if beast:FindFirstChild("HumanoidRootPart") and beast:FindFirstChild("Máu") and beast.Health.Value > 0 then
...
local function GetFruitStock()
    ...
    if success and advancedStock then
        ...
        result = result .. "- Lỗi retrieving data.\n"
    end
...
-- ========================================
v9:AddSection({"Chức năng Trái tự động"})

-- Auto Random Fruit
v9:AddToggle({
    Name = "Tự động lấy Trái ngẫu nhiên",
...
-- Auto Buy Fruit Shop
v9:AddToggle({
    Name = "Tự động mua Trái (Săn shop)",
...
local FruitCheckResult = v9:AddParagraph({
    Title = "Trạng thái Trái",
    Desc = "Chọn Trái để kiểm tra"
})

-- Button check fruit
v9:AddButton({
    Name = "Kiểm tra Trái đã chọn",
    Callback = function()
...
v9:AddToggle({
    Name = "Tự động thông báo khi Trái có hàng",
...
-- RAID STATUS
-- ========================================
v10:AddSection({"Trạng thái Đột kích"})
...
v10:AddButton({
    ...
    Callback = function()
        if not GetBP("Special Microchip") and _G.SelectChip then
            pcall(function()
                replicated.Remotes.CommF_:InvokeServer("RaidsNpc", "Chọn", _G.SelectChip)
            end)
        end
    end
})
...
_G.AutoStartRaid = GetSetting("Raid_AutoBắt đầuRaid", false)
v10:AddToggle({
    Name = "Tự động bắt đầu Đột kích",
    Default = _G.AutoStartRaid,
    Callback = function(Value)
        _G.AutoStartRaid = Value
        _G.SaveData["Raid_AutoBắt đầuRaid"] = Value
        SaveSettings()
    end
})
...
-- ========================================
v10:AddSection({"Tự động Farm Hầm ngục"})
...
v10:AddToggle({
    Name = "Tự động Farm Hầm ngục + Tầng tiếp theo",
...
-- TELEPORT FLOORS
-- ========================================
v10:AddSection({"Dịch chuyển các tầng"})

v10:AddButton({
    Name = "TP đến đầu Tầng 1",
    Callback = function()
        if World2 then
...
_G.AutoStartLawRaid = GetSetting("Raid_AutoBắt đầuLawRaid", false)
v10:AddToggle({
    Name = "Tự động bắt đầu Đột kích Law",
    Default = _G.AutoStartLawRaid,
    Callback = function(Value)
        _G.AutoStartLawRaid = Value
        _G.SaveData["Raid_AutoBắt đầuLawRaid"] = Value
        SaveSettings()
    end
})
...
local Location_Portal = {
    ...
    "Lâu đài trên biển", "Mansion Cafe", "Dịch chuyển Hydra", "Canvendish Room", "Đền Thời gian"
}
...
v11:AddButton({ 
    ...
    Callback = function()
        ...
        elseif _G.Island_PT == "Lâu đài trên biển" then
        ...
        elseif _G.Island_PT == "Mansion Cafe" then
            replicated.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-12471.169921875, 374.94024658203, -7551.677734375))
        elseif _G.Island_PT == "Dịch chuyển Hydra" then
        ...
        elseif _G.Island_PT == "Đền Thời gian" then
...
v13:AddToggle({
    Name = "ESP Người chơi",
    Default = GetSetting("ESP_Người chơi_Save", false),
    Callback = function(Value)
        PlayerESPEnabled = Value
        _G.SaveData["ESP_Người chơi_Save"] = Value
        SaveSettings()
    end
})
-- Island ESP
            ...
            while _G.AutoBuyMelee do
                ...
                if lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if dist > 15 then
                        ...
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", data.Key)
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "Dragonheart")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "Dragonstorm")
...
v14:AddButton({
    Name = "Craft DinoHood",
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "DinoHood")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "SharkTooth")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "TerrorJaw")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "SharkAnchor")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "LeviathanCrown")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "LeviathanShield")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "LeviathanBoat")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "LegendaryScroll")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftVật phẩm", "Craft", "MythicalScroll")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Cutlass")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Katana")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Iron Mace")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Duel Katana")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Triple Katana")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Pipe")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Dual-Headed Blade")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Bisento")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Soul Cane")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Slingshot")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Musket")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Flintlock")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Refined Flintlock") -- Sửa tên cho đúng
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Refined Flintlock")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Cannon")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Black Cape")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Swordsman Hat")
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyVật phẩm", "Tomoe Ring")
...
v15:AddSection({"Chức năng máy chủ"})
...
v15:AddButton({
    ...
    Callback = function()
        ...
        if not RedeemRemote then
            redzlib:Notify({Title = "Lỗi", Message = "Không tìm thấy remote Redeem", Duration = 3})
            return
        end
        ...
        redzlib:Notify({Title = "Thành công", Message = "Đã thử nhập tất cả mã", Duration = 3})
    end
})
...
-- 2. PLAYER GUI / OTHERS
-- ========================================
v15:AddSection({"Giao diện người chơi / Khác"})
...
v15:AddToggle({
    ...
    Callback = function(Value)
        ...
        SaveSettings()
        pcall(function()
            game:GetService("Bắt đầuerGui"):SetCoreGuiEnabled(Enum.CoreGuiType.Chat, not Value)
        end)
    end
})
...
v15:AddToggle({
    ...
    Callback = function(Value)
        ...
        SaveSettings()
        pcall(function()
            game:GetService("Bắt đầuerGui"):SetCoreGuiEnabled(Enum.CoreGuiType.PlayerList, not Value)
        end)
    end
})
...
v15:AddButton({
    ...
    Callback = function()
        ...
        redzlib:Notify({Title = "Graphics", Message = "Đã chuyển sang chế độ Fast Chế độ", Duration = 2})
    end
})

-- Low CPU Mode
v15:AddButton({
    Name = "Chế độ CPU thấp",
    Callback = function()
        LowCpu()
...
v15:AddButton({
    Name = "Tăng tốc độ thuyền (Client)",
    Callback = function()
        pcall(function()
...
-- 5. CONFIGURE - GOD
-- ========================================
v15:AddSection({"Cấu hìnhure - God"})
...
local function updatePlayerList()
    local newList = {}
    for _, p in ipairs(game:GetService("Người chơi"):GetPlayers()) do
...
local PlayerDropdown = v12:AddDropdown({
    ...
    Callback = function(value)
        ...
        -- Lưu cấu hình
        _G.SaveData["Người chơiList"] = value
        SaveSettings()
    end
})
    ...
    Callback = function(value)
        ...
        task.spawn(function()
            while _G.TpPly and _G.PlayersList do
                pcall(function()
                    local target = game:GetService("Người chơi"):FindFirstChild(_G.PlayersList)
...
-- ========================================
v12:AddSection({"Cài đặt LocalNgười chơi / Khác"})
...
v12:AddToggle({
    Name = "Phạm vi quan sát Instance [ VÔ HẠN ]",
...
-- ========================================
v12:AddSection({"Cài đặt chiến đấu / Cài đặt Aimbot"})
