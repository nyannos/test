...
-- ========================================
-- SWITCH HUB UI (Black & White) + shim Tabs.* + Minimize
-- ========================================
print("[nyann os] loading Switch Hub UI...")

-- Embedded B&W Switch Hub library
do
-- Switch Hub UI | Black & White theme
...
_G.ColorButton = Color3.fromRGB(20, 20, 20)
IKAI = true
if IKAI then
	do
...
if not library then
  error("[nyann os] Switch Hub library failed to load")
end
print("[nyann os] Switch Hub OK (B&W)")
...
local function MakeTabShim(tabPage)
  ...
  function tab:AddSection(name)
    ...
    pcall(function()
      if tabPage.Seperator then
        tabPage:Seperator(name)
      elseif tabPage.Label then
        tabPage:Label(name)
      end
    end)
    return tabPage
  ...
  function tab:AddToggle(cfg)
    ...
    local icon = "9606294253"
    local ret
    pcall(function()
      ret = tabPage:Toggle(name, icon, def, cb)
    end)
    return ret
  end
  ...
  function tab:AddParagraph(a, b)
    ...
    local proxy = {
      _title = title,
      Set = function(self, t) self._title = tostring(t or "") pcall(function() if self._lbl then self._lbl:Set(self._title) end end) end,
      SetDesc = function(self, d) end,
      SetDescription = function(self, d) end,
    }
    pcall(function()
      if tabPage.Label then
        proxy._lbl = tabPage:Label(title .. (desc ~= "" and (" | " .. desc) or ""))
      end
    end)
    return proxy
  end
  ...
  function tab:AddTextBox(cfg)
    ...
    local cb = cfg.Callback or function() end
    pcall(function()
      if tabPage.Textbox then
        tabPage:Textbox(name, false, cb)
      end
    end)
  end
...
function Window:MakeTab(cfg)
  ...
  local icon = "7040391851"
  if type(cfg.Icon) == "string" then
    local n = cfg.Icon:gsub("rbxassetid://", "")
    if tonumber(n) then icon = n end
  elseif type(cfg.Icon) == "number" then
    icon = tostring(cfg.Icon)
  end
  local page
  local ok, res = pcall(function()
    return RealWindow:AddTab(title, icon)
  end)
...
-- Toggle menu (RippleHUBLIB)
local function ToggleSwitchMenu(force)
...
getgenv().NyannToggleMenu = ToggleSwitchMenu
...
print("[nyann os] Switch Hub menu ready")
