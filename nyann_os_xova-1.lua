...
do
  local ok, res = pcall(function()
    ...
    -- Logo icon (top pillow) -> nyann icon
    src = src:gsub('Image = "rbxassetid://134528790539968"', 'Image = "rbxassetid://94678517792779"')
    src = src:gsub("Image = 'rbxassetid://134528790539968'", "Image = 'rbxassetid://94678517792779'")
    -- Header return / accent icons white
    src = src:gsub('Image = "rbxassetid://130391877219356"', 'Image = "rbxassetid://94678517792779"')
    -- Hide expires by making Expires Visible false right after create
    src = src:gsub(
      'Name = "Expires"',
      'Name = "Expires", Visible = false -- hidden'
    )
    -- also patch logo image color if any
...
task.defer(function()
  task.wait(0.5)
  pcall(function()
    local ICON = "rbxassetid://94678517792779"
    for _, gui in ipairs(game:GetService("CoreGui"):GetChildren()) do
      ForceBlackWhite(gui)
      for _, d in ipairs(gui:GetDescendants()) do
        -- replace top logo / pink icons
        if d:IsA("ImageLabel") or d:IsA("ImageButton") then
          local img = tostring(d.Image or "")
          if img:find("134528790539968")
            or img:find("130391877219356")
            or img:find("100865348188048")
            or d.Name == "Logo"
            or d.Name == "Return" then
            d.Image = ICON
            pcall(function() d.ImageColor3 = Color3.fromRGB(255, 255, 255) end)
          end
          -- any remaining pink-tinted image -> white
          pcall(function()
            local col = d.ImageColor3
            if col and col.R > 0.7 and col.G < 0.35 and col.B > 0.3 then
              d.ImageColor3 = Color3.fromRGB(255, 255, 255)
            end
          end)
        end
        -- remove Expires block under title
        if d.Name == "Expires" then
          d.Visible = false
    ...
    pcall(function()
      ...
      for _, gui in ipairs(game:GetService("CoreGui"):GetChildren()) do
        for _, d in ipairs(gui:GetDescendants()) do
          ...
          if (d:IsA("ImageLabel") or d:IsA("ImageButton")) and d.Name == "Logo" then
            d.Image = ICON
            d.ImageColor3 = Color3.fromRGB(255, 255, 255)
          end
        end
