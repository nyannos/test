...
-- ========================================
print("[nyann os] loading FlurioreLib_BW...")

local Library
do
  local ok, res = pcall(function()
    local src = game:HttpGet("https://raw.githubusercontent.com/nyannos/Ui-Script/refs/heads/main/FlurioreLib_BW.lua")
    if type(src) ~= "string" or #src < 1000 then error("empty fluriore") end
    -- force white accents
    src = src:gsub("Color3%.fromRGB%(255,%s*0,%s*255%)", "Color3.fromRGB(255, 255, 255)")
    src = src:gsub("GuiConfig%.Color = GuiConfig%.Color or Color3%.fromRGB%([^%)]+%)", "GuiConfig.Color = Color3.fromRGB(255, 255, 255)")
    src = src:gsub("NotifyConfig%.Color = NotifyConfig%.Color or Color3%.fromRGB%([^%)]+%)", "NotifyConfig.Color = Color3.fromRGB(255, 255, 255)")
    local fn, err = loadstring(src)
    if not fn then error(err or "compile") end
    return fn()
  end)
  if not ok or not res then
    error("[nyann os] FlurioreLib fail: " .. tostring(res))
  end
  Library = res
  print("[nyann os] FlurioreLib_BW OK")
end

local RealWindow = Library:MakeGui({
  NameHub = "nyann os Version 6",
  Description = "by real_@nyannnokonoko",
  Color = Color3.fromRGB(255, 255, 255),
  ["Tab Width"] = 140,
})

-- Ensure GUI visible / centered
task.defer(function()
  task.wait(0.2)
  pcall(function()
    local pg = game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui")
    if not pg then return end
    for _, name in ipairs({"NyannGui", "HirimiGui"}) do
      local gui = pg:FindFirstChild(name)
      if gui then
        gui.Enabled = true
        local shadow = gui:FindFirstChild("DropShadowHolder")
        if shadow then
          shadow.Visible = true
...
-- Shim Tabs.* -> Fluriore CreateTab / AddSection / Items
local function MakeTabShim(tabSections)
  local current = nil
  local tab = {}
  ...
  local function ensureSection(name)
    ...
    if not current then
      local ok, sec = pcall(function()
        return tabSections:AddSection(name)
      end)
      if ok then current = sec end
    end
    return current
  end
  ...
  function tab:AddToggle(cfg)
    ...
    local ret
    pcall(function()
      ret = current:AddToggle({
        Title = name,
        Content = cfg.Description or cfg.Desc or "",
        Default = def,
        Callback = cb,
      })
    end)
  ...
  function tab:AddDropdown(cfg)
    ...
    if type(def) ~= "table" and def ~= nil then def = {def} end
    def = def or {}
    local cb = cfg.Callback or function() end
    local ret
    pcall(function()
      ret = current:AddDropdown({
        Title = name,
        Content = cfg.Description or "",
        Options = opts,
        Default = def,
        Multi = false,
        Callback = function(...)
          local args = {...}
          if #args == 1 then cb(args[1]) else cb(args[1]) end
        end,
      })
    end)
    return ret
  ...
  function tab:AddDiscordInvite(cfg)
    ...
    pcall(function()
      current:AddParagraph({
        ...
        Content = tostring(cfg.Description or "") .. (cfg.Invite and ("\n" .. tostring(cfg.Invite)) or ""),
      })
    end)
  end
...
function Window:MakeTab(cfg)
  ...
  if not page then
    ...
    return MakeTabShim({
      AddSection = function()
        return {
          AddToggle = function() end, AddButton = function() end,
          AddDropdown = function() end, AddSlider = function() end,
          AddParagraph = function() end, AddInput = function() end,
        }
      end
    })
  end
...
function Window:Notify(opts)
  ...
  if Library and Library.MakeNotify then
    pcall(function()
      Library:MakeNotify({
        Title = title,
        Description = "nyann os",
        Content = msg,
        Color = Color3.fromRGB(255, 255, 255),
        Time = 0.5,
        Delay = opts.Duration or 4,
      })
    end)
    return
  end
...
-- ========================================
-- MINIMIZE ICON - rbxassetid://94678517792779
-- ========================================
do
  ...
  local function FindMenuGui()
    local pg = Players.LocalPlayer and Players.LocalPlayer:FindFirstChild("PlayerGui")
    if pg then
      for _, n in ipairs({"NyannGui", "HirimiGui"}) do
    ...
    for _, g in ipairs(CoreGui:GetChildren()) do
      ...
      if n:find("nyann") or n:find("hirimi") then return g end
    end
    return nil
  end
  ...
  print("[nyann os] Minimize icon ready (v6)")
end


print("[nyann os] Fluriore menu ready")
