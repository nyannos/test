...
-- Language pack (EN / VI)
_G.Language = _G.Language or "vi"
local LANG = {
    vi = {
        welcome_title = "Chào mừng đến nyann os!",
        welcome_content = "Cảm ơn bạn đã sử dụng",
        beta_title = "Script beta nên còn lỗi nhé",
        beta_content = "",
        lang_changed = "Đã đổi ngôn ngữ: Tiếng Việt",
        lang_hint = "Một số chữ cần chạy lại script để áp dụng hết",
        select_lang = "Chọn ngôn ngữ",
        apply_lang = "Áp dụng ngôn ngữ",
        font_section = "Font",
        select_font = "Chọn Font",
        apply_font = "Áp dụng Font",
        full_bright = "Full Bright",
        no_anim = "Tắt Animation",
        misc_graphics = "Đồ họa / Misc",
        language = "Ngôn ngữ",
    },
    en = {
        welcome_title = "Welcome to nyann os!",
        welcome_content = "Thanks for using",
        beta_title = "Beta script — may have bugs",
        beta_content = "",
        lang_changed = "Language set: English",
        lang_hint = "Re-run script to apply all labels fully",
        ...
        misc_graphics = "Graphics / Misc",
        language = "Language",
    },
}

local function T(key)
    local pack = LANG[_G.Language] or LANG.vi
    return pack[key] or (LANG.en[key] or key)
end
...
-- Map text VI <-> EN for live GUI update
local TEXT_MAP = {
    -- tabs
    ["Information"] = { vi = "Information", en = "Information" },
    ["Status/Server"] = { vi = "Status/Server", en = "Status/Server" },
    ["Farming"] = { vi = "Farming", en = "Farming" },
    ["Quests & Other"] = { vi = "Nhiệm vụ & Khác", en = "Quests & Other" },
    ["Settings Farm"] = { vi = "Cài đặt Farm", en = "Settings Farm" },
    ["Auto Fishing"] = { vi = "Tự động câu cá", en = "Auto Fishing" },
    ["Sea Event"] = { vi = "Sự kiện biển", en = "Sea Event" },
    ["Volcano & Dojo"] = { vi = "Núi lửa & Dojo", en = "Volcano & Dojo" },
    ["Mirage & Race"] = { vi = "Mirage & Race", en = "Mirage & Race" },
    ["Fruits & Check Stock"] = { vi = "Trái & Kiểm tra kho", en = "Fruits & Check Stock" },
    ["Raid & Dungoen"] = { vi = "Raid & Dungeon", en = "Raid & Dungeon" },
    ["Miscellaneous"] = { vi = "Khác", en = "Miscellaneous" },
    -- common toggles
    ["Full Bright"] = { vi = "Sáng tối đa", en = "Full Bright" },
    ["Fast Mode (Low Graphics)"] = { vi = "Chế độ nhanh (đồ họa thấp)", en = "Fast Mode (Low Graphics)" },
    ["Low CPU Mode"] = { vi = "Chế độ tiết kiệm CPU", en = "Low CPU Mode" },
    ["Remove Sky Fog"] = { vi = "Xóa sương mù", en = "Remove Sky Fog" },
    ["Disable Leaderboard GUI"] = { vi = "Tắt bảng xếp hạng", en = "Disable Leaderboard GUI" },
    ["Teleport to Player"] = { vi = "Dịch chuyển tới người chơi", en = "Teleport to Player" },
    ["Aimbot Cam Lock"] = { vi = "Khóa camera Aimbot", en = "Aimbot Cam Lock" },
    ["Set WalkSpeed"] = { vi = "Đặt tốc độ chạy", en = "Set WalkSpeed" },
    ["Set JumpPower"] = { vi = "Đặt lực nhảy", en = "Set JumpPower" },
    ["Select Language"] = { vi = "Chọn ngôn ngữ", en = "Select Language" },
    ["Chọn ngôn ngữ"] = { vi = "Chọn ngôn ngữ", en = "Select Language" },
    ["Apply Language"] = { vi = "Áp dụng ngôn ngữ", en = "Apply Language" },
    ["Áp dụng ngôn ngữ"] = { vi = "Áp dụng ngôn ngữ", en = "Apply Language" },
}

local function ApplyLanguageToGui(lang)
    lang = lang or _G.Language or "vi"
    local parents = {}
    pcall(function()
    ...
    pcall(function()
        ...
        if lp and lp:FindFirstChild("PlayerGui") then
            table.insert(parents, lp.PlayerGui)
        end
    end)
            ...
            for _, obj in ipairs(parent:GetDescendants()) do
                if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                    ...
                    if type(txt) == "string" and #txt > 0 then
                        local mapped = nil
                        if TEXT_MAP[txt] and TEXT_MAP[txt][lang] then
                            mapped = TEXT_MAP[txt][lang]
                        else
                            for _, pair in pairs(TEXT_MAP) do
                                if txt == pair.vi or txt == pair.en then
                                    mapped = pair[lang]
                                    break
                                end
                            end
                        end
                        if mapped and mapped ~= txt then
...
-- Welcome
pcall(function()
    -- load saved lang early if settings already loaded later; default vi
    NyannNotify(T("welcome_title"), T("welcome_content"), {
...
task.wait(2)
pcall(function()
    NyannNotify(T("beta_title"), T("beta_content"), {
...
-- restore language from save
pcall(function()
    local saved = GetSetting("Language_Save", "vi")
    if saved == "en" or saved == "vi" then
        _G.Language = saved
    end
end)
...
-- ========================================
-- LANGUAGE (EN / VI) — Miscellaneous
-- ========================================
v15:AddSection({"Language / Ngôn ngữ"})

local LangOptions = {"Tiếng Việt", "English"}
local function LangCodeFromLabel(label)
    if label == "English" then return "en" end
    return "vi"
end
local function LabelFromLangCode(code)
    return (code == "en") and "English" or "Tiếng Việt"
end

v15:AddDropdown({
    Name = "Select Language / Chọn ngôn ngữ",
    Options = LangOptions,
    Default = LabelFromLangCode(GetSetting("Language_Save", "vi")),
    Callback = function(Value)
        local code = LangCodeFromLabel(Value)
        _G.Language = code
        _G.SaveData["Language_Save"] = code
        SaveSettings()
        pcall(function() ApplyLanguageToGui(code) end)
        pcall(function()
            NyannNotify(T("lang_changed"), T("lang_hint"), {
                Length = 4,
                BarColor = Color3.fromRGB(75, 200, 120),
                Image = "rbxassetid://6031068421",
                Audio = "rbxassetid://17208361335",
            })
        end)
...
v15:AddButton({
    Name = "Apply Language / Áp dụng",
    Callback = function()
        local code = _G.Language or GetSetting("Language_Save", "vi")
        pcall(function() ApplyLanguageToGui(code) end)
        pcall(function()
            NyannNotify(T("lang_changed"), T("lang_hint"), {
                Length = 3,
...
-- apply once after menu built
task.delay(2, function()
    pcall(function()
        local code = GetSetting("Language_Save", "vi")
        _G.Language = code
        ApplyLanguageToGui(code)
    end)
end)
